export type CanonicalTaskPrincipalType = 'person' | 'service' | 'internal';

export type CanonicalTaskPrincipal = Readonly<{
  type: CanonicalTaskPrincipalType;
  id: string;
}>;

const ALLOWED_TYPES = new Set<CanonicalTaskPrincipalType>(['person', 'service', 'internal']);
...
export function normalizeCanonicalTaskPrincipal(principal: {
  type?: unknown;
  id?: unknown;
} | null | undefined): CanonicalTaskPrincipal {
  const type = String(principal?.type || '').toLowerCase() as CanonicalTaskPrincipalType;
...
export function createCanonicalTaskPrincipal({
  authSource,
  personId,
  serviceId,
  internalId,
...
export function principalNamespace(principal: CanonicalTaskPrincipal): string {
