...
import { retrieveGraph, type GraphRetrievalInput, type GraphRetrievalResponse } from './graph-retrieval.js';
...
export const PORTABLE_GRAPH_SCHEMA_VERSION = 1 as const;
 ...
 */
export function createPortableGraph(os: PersonalOs): PortableGraphBundle {
  ...
  const bundle: PortableGraphBundle = {
 ...
 */
export function validatePortableGraph(value: unknown): asserts value is PortableGraphBundle {
  assertJsonValue(value, '$', new WeakSet<object>(), 0);
  if (!isPlainRecord(value)) throw new Error('PORTABLE-GRAPH-SHAPE: bundle must be a plain JSON object');
  ...
  const graph = value.graph as unknown as GraphFileV2;
  if (graph.version !== 2) {
...
export function hydratePortableGraph(bundle: PortableGraphBundle): PersonalOs {
  validatePortableGraph(bundle);
  return {
    dataDir: '',
...
export function portableGraphDigest(value: PortableGraphBundle): string {
...
function validateDecisionRecords(graph: GraphFileV2, decisions: DecisionRecord[]): void {
  ...
  decisions.forEach((value, index) => {
    if (!isPlainRecord(value)) throw new Error(`PORTABLE-GRAPH-DECISION-SHAPE at decisions[${index}]`);
    ...
    if (records.has(value.id)) throw new Error(`PORTABLE-GRAPH-DECISION-ID-UNIQUE: duplicate decision ID ${value.id}`);
    ...
    if (value.supersedes !== undefined) {
      ...
      for (const target of value.supersedes) {
        if (refs.has(target)) throw new Error(`PORTABLE-GRAPH-DECISION-REFERENCE: duplicate supersedes reference ${target}`);
        refs.add(target);
      }
    }
  });
  ...
  for (const id of decisionEntities) {
    if (!records.has(id)) throw new Error(`PORTABLE-GRAPH-DECISION-REFERENCE: missing decision record for Graph entity ${id}`);
  }
...
function requireNonEmptyString(value: unknown, path: string): asserts value is string {
  if (typeof value !== 'string' || value.trim() === '') throw new Error(`PORTABLE-GRAPH-STRING at ${path}`);
}

function optionalString(value: unknown, path: string): void {
  if (value !== undefined && typeof value !== 'string') throw new Error(`PORTABLE-GRAPH-STRING at ${path}`);
}
...
function assertJsonValue(value: unknown, path: string, active: WeakSet<object>, depth: number): void {
  if (depth > PORTABLE_GRAPH_MAX_JSON_DEPTH) throw new Error(`PORTABLE-GRAPH-BOUND: JSON depth exceeds ${PORTABLE_GRAPH_MAX_JSON_DEPTH} at ${path}`);
  if (value === null || typeof value === 'string' || typeof value === 'boolean') return;
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new Error(`PORTABLE-GRAPH-JSON: non-finite number at ${path}`);
    return;
  }
  if (typeof value !== 'object') throw new Error(`PORTABLE-GRAPH-JSON: non-JSON value at ${path}`);
  if (active.has(value)) throw new Error(`PORTABLE-GRAPH-JSON: cyclic value at ${path}`);
  active.add(value);
  try {
    if (Array.isArray(value)) {
      for (let index = 0; index < value.length; index += 1) {
        if (!(index in value)) throw new Error(`PORTABLE-GRAPH-JSON: sparse array at ${path}[${index}]`);
        assertJsonValue(value[index], `${path}[${index}]`, active, depth + 1);
      }
      return;
    }
    if (!isPlainRecord(value)) throw new Error(`PORTABLE-GRAPH-JSON: value at ${path} must be a plain JSON object`);
    for (const key of Object.getOwnPropertyNames(value)) {
      const descriptor = Object.getOwnPropertyDescriptor(value, key);
      if (!descriptor || !('value' in descriptor)) throw new Error(`PORTABLE-GRAPH-JSON: accessor at ${path}.${key}`);
      assertJsonValue(descriptor.value, `${path}.${key}`, active, depth + 1);
    }
    if (Object.getOwnPropertySymbols(value).length > 0) throw new Error(`PORTABLE-GRAPH-JSON: symbol key at ${path}`);
  } finally {
    active.delete(value);
  }
}
...
function stableJson(value: unknown, depth: number): string {
  if (depth > PORTABLE_GRAPH_MAX_JSON_DEPTH) throw new Error(`PORTABLE-GRAPH-BOUND: JSON depth exceeds ${PORTABLE_GRAPH_MAX_JSON_DEPTH}`);
