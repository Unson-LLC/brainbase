...
export const knowledgeResolutionTools: Tool[] = [{
...
export async function handleKnowledgeResolutionToolCall(name: string, args: Record<string, unknown>, dependencies: Dependencies): Promise<ToolResult | null> {
  if (name !== 'brainbase_knowledge_resolve') return null;
  const context = await authenticateProject(args, dependencies);
  if ('status' in context) return context;
  const fetched = await fetchAuthenticatedJson(dependencies, context, { path: '/api/knowledge/resolve', method: 'POST', body: args });
  ...
  if (!response.ok) {
    ...
    return toolError(response.status >= 500 ? 'unavailable' : 'error', response.status >= 500 ? 'brainbase_api_unavailable' : 'brainbase_api_error', message, context.scope, response.status);
  }
  if (!isKnowledgeResolutionReceipt(payload)) {
    return toolError('error', 'brainbase_api_response_invalid', 'Brainbase API returned an invalid knowledge resolution receipt', context.scope, response.status);
  }
