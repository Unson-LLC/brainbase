...
describe('MCP tool capability annotations', () => {
  it('classifies every published tool explicitly', () => {
    const expectedReadOnly: Record<string, boolean> = {
      ...
      brainbase_onboarding_get: true, brainbase_knowledge_resolve: true, brainbase_knowledge_retrieve: true, brainbase_resolve_turn: true,
      brainbase_judgment_audit_read: true, brainbase_knowledge_evidence_record: true,
      brainbase_personal_kg_answer_record: true,
      brainbase_judgment_node_record: true,
      brainbase_get_meeting_minutes_context: true, brainbase_get_shareable_person_profile: true,
      authorize_tenant_resource: true, mesh_peers: true,
  ...
  it('marks representative readers as read-only and mutations as writes', () => {
    ...
    for (const name of ['register_personal_kg', 'brainbase_onboarding_review', 'brainbase_automation_human_step_resolve', 'create_task', 'update_task', 'mesh_query', 'graph_export_snapshot', 'graph_record_human_gate_receipt', 'graph_plan_mutations', 'graph_apply_plan', 'graph_rollback_plan']) {
