...
function normalizeStoreInput(input) {
    ...
    assertNoUnknownFields(input, new Set([
        'operation', 'tenant_id', 'connection_id', 'connection_revision', 'provider',
...
function normalizeReferenceInput(input, { reason = false } = {}) {
    ...
    const fields = new Set([
        'operation', 'tenant_id', 'connection_id', 'connection_revision', 'provider', 'credential_ref',
        ...(reason ? ['reason'] : [])
    ]);
    ...
    return { ...binding, credential_ref: credentialRef, ...(reason ? { reason: input.reason ?? null } : {}) };
}
