...
- **Judgment Resolver**: 各Codex turnはglobal Hostが1つのjudgment episodeとして管理する。`UserPromptSubmit`で生の会話履歴・current request・prior finalized episode・runtime/instruction bindingからcanonical contextを作り、model生成前に1つのjudgment episodeを開始して初期route receiptを採用する。modelはResolverを呼ばず、分類や文脈を作らない。返された`active_node_definitions`だけを実行し、Brainbase knowledge/retrieval toolは結果を踏まえて0..N回呼べる。`PostToolUse`は実際のBrainbase callを記録し、`Stop`がepisode receiptを1件だけ確定する。required `knowledge.resolve`が未実行なら最初のStopで継続し、再Stopではincompleteとして確定して無限loopを防ぐ。clarification receiptでも回答生成へ進む。project bindingは判断文脈であり、project access不能だけで判断を止めない。episode receiptはaction許可ではなく、通常の権限・承認を置き換えない。詳細は`brainbase-judgment-resolver`。
