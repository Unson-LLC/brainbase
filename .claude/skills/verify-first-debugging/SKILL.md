...
## Evidence Hierarchy（根拠の優先順）

原因判定は**証拠階層順**で検証する。外部理論の前に、内部の事実を積み上げる。

1. ユーザーの時間的主張（最近の変更）
2. 内部コード優先（関連ハンドラーを完全に読む）
3. ログ・トレース
4. 外部理論は最後（証拠が揃った後）

## 症状ループ検知 → 根本原因調査へ切替

- **「同じ症状が繰り返し起きる」**は、対症療法を止める合図。
- process kill / 再起動で凌ぐのではなく、**起動ロジック・実行時引数・環境差分**に切り替えて調査する。

## 実行時の現実確認（コマンドライン）

- **コード上の引数配列が正しくても、実行時に欠落する**ことがある。
- `ps aux` などで**実際のコマンドライン**を確認し、途中で切れていないかを検証する。
- 例: `-t scrollSensitivity` で止まり、後続引数が欠落している場合は **spawn時の実引数異常**。
