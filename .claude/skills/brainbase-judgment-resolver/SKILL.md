...
1. 回答やactionの前に、そのturnの現在の問い、project、分類proposalを `brainbase_judgment_resolve` へ渡す。追従発話では、意味解決に必要な先行発話だけを`conversation_context.text`と`source_turn_ids`で明示する。
