...
3. Hostは通常turnではmodel生成前にResolver bridgeを呼び、request/contextへ束縛された初期route receiptを採用して1つのjudgment episodeを開始する。通常開始は`episode_origin=user_prompt_submit`、`route_application=pre_generation`を記録する。Codex App委任で`UserPromptSubmit`が発火しなかった場合だけ、最初の`Stop`は現在turnの完全な`codex_delegation`を一意に復元できる。復元元は`codex_app`の`create_thread`/`send_message_to_thread`と現在session aliasの連結componentに限定し、別session component、複数候補、壊れた包みはfail-closedとする。復元episodeは`episode_origin=stop_delegation_recovery`、`route_application=post_generation_recovery`を記録し、routeは差し戻しと後続処理だけを支配する。すでに生成された初回回答を事前に導いたとは主張しない。開始前のtransport retryは許すが、開始後は再分類しない。modelから呼べるJudgment Resolver toolは公開しない。
