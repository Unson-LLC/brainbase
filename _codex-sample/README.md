...
`_codex-sample/` は、brainbaseの構造とデータ形式を理解するためのサンプルです。実際の環境では、`_codex/` ディレクトリに本番データが格納されますが、このサンプルは開発・テスト・学習目的で使用します。

**重要**: このディレクトリはOSS公開用のサンプルです。実際のプロジェクト・組織・人物データは含まれていません。
**補足**: 情報SSOT（Decision/RACI/Policy）はPostgresに移行可能な設計であり、`_codex-sample` は人間向けビューのサンプルとして扱います。
