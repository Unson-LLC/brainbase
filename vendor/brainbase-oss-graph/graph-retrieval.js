 ...
 */
export async function retrieveGraph(os, input, provider) {
    const normalized = normalizeInput(input);
    validateCanonicalGraph(os.graph);
    if (os.graph.version === 1)
        return migrationResponse(normalized);
    ...
    if (traversed.length > ranked.length)
        partialReasons.push('result_limit_truncated');
    ...
    if (missingEvidence.length > 0)
        partialReasons.push('missing_evidence');
    const sufficiency = evidence.length > 0 ? 'needs_model_verification' : 'insufficient';
    const coverage = partialReasons.length > 0 ? 'partial' : 'complete';
...
function normalizeInput(input) {
    if (!input || typeof input !== 'object')
        throw new Error('GRAPH-RETRIEVAL-INPUT: input must be an object');
    if (typeof input.query !== 'string' || input.query.trim() === '')
        throw new Error('GRAPH-RETRIEVAL-QUERY: query must be a non-empty string');
    const asOf = input.asOf ?? new Date().toISOString();
    isActiveAt({}, asOf);
    const partialReasons = [];
    const requestedLimit = input.limit ?? 10;
    if (!Number.isInteger(requestedLimit) || requestedLimit < 1)
        throw new Error('GRAPH-RETRIEVAL-LIMIT: limit must be a positive integer');
    const limit = Math.min(requestedLimit, GRAPH_RETRIEVAL_MAX_LIMIT);
    if (requestedLimit > GRAPH_RETRIEVAL_MAX_LIMIT)
        partialReasons.push('limit_capped_max_50');
    ...
    if (allSeeds.length > seedIds.length)
        partialReasons.push('seed_ids_capped_max_10');
    if (input.steps !== undefined && !Array.isArray(input.steps))
        throw new Error('GRAPH-RETRIEVAL-STEPS: steps must be an array');
    ...
    if (allSteps.length > steps.length)
        partialReasons.push('steps_capped_max_3');
    return { ...input, query: input.query.trim(), asOf, limit, seedIds, steps, partialReasons };
}
function normalizeStep(step, index) {
    if (!step || typeof step !== 'object')
        throw new Error(`GRAPH-RETRIEVAL-STEP-${index}: step must be an object`);
    try {
        getCanonicalRelation(step.relation);
    }
    catch (error) {
        throw new Error(`GRAPH-RETRIEVAL-STEP-${index}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
function assertProviderId(provider) {
    if (typeof provider.id !== 'string' || provider.id.trim() === '')
        throw new Error('GRAPH-RETRIEVAL-EMBEDDING-PROVIDER: provider.id must be a non-empty string');
    return provider.id;
}
async function discoverSemantic(scoped, query, _limit, provider) {
...
function validateVector(value, index, providerId) {
    if (!Array.isArray(value) || value.length === 0)
        throw new Error(`GRAPH-RETRIEVAL-EMBEDDING-VECTOR: provider ${providerId} returned an empty vector at index ${index}`);
    ...
    if (Math.hypot(...value) === 0)
        throw new Error(`GRAPH-RETRIEVAL-EMBEDDING-ZERO: provider ${providerId} returned a zero vector at index ${index}`);
    return value;
}
function cosine(left, right) {
    if (left.length !== right.length)
        throw new Error(`GRAPH-RETRIEVAL-EMBEDDING-DIMENSION: vectors have dimensions ${left.length} and ${right.length}`);
    let dot = 0;
    let leftNorm = 0;
    let rightNorm = 0;
    ...
    if (!Number.isFinite(denominator) || denominator === 0)
        throw new Error('GRAPH-RETRIEVAL-EMBEDDING-COSINE: cannot calculate cosine similarity');
    return dot / denominator;
}
function discoverLexical(scoped, query, _limit) {
    return scoped
        .map((item) => ({
        entity: item.entity,
...
function traverse(graph, scoped, seeds, steps, activeIds, asOf, partialReasons) {
    let frontier = seeds;
    for (const [stepIndex, step] of steps.entries()) {
        ...
        for (const current of frontier) {
            for (const edge of graph.edges) {
                ...
                if (!targetId || !activeIds.has(edge.fromId) || !activeIds.has(edge.toId) || !isActiveAt(edge, asOf))
                    continue;
                ...
                if (!prior || compareStates(state, prior) < 0)
                    next.set(targetId, state);
            }
        }
        if (next.size === 0)
            partialReasons.push(`no_matching_edge_for_step_${stepIndex + 1}`);
...
function edgeTarget(edge, step, currentId) {
    if (edge.relation !== step.relation)
        return undefined;
    ...
    if (step.targetType && step.targetType !== targetType)
        return undefined;
    return currentType && targetType ? targetId : undefined;
}
function observeRelations(graph, seedIds, visibleIds, asOf) {
    const observed = new Map();
    const seedSet = new Set(seedIds);
    for (const edge of graph.edges) {
        if (!visibleIds.has(edge.fromId) || !visibleIds.has(edge.toId) || !isActiveAt(edge, asOf))
            continue;
        const relation = getCanonicalRelation(edge.relation);
        const observations = [];
        if (seedSet.has(edge.fromId))
            observations.push({ direction: 'outgoing', targetId: edge.toId, targetType: relation.to });
        if (seedSet.has(edge.toId))
            observations.push({ direction: 'incoming', targetId: edge.fromId, targetType: relation.from });
        for (const observation of observations) {
            ...
            if (!existing.targetIds.includes(observation.targetId))
                existing.targetIds.push(observation.targetId);
...
function toCandidate(state, bundle) {
...
function evidenceFor(os, entity) {
    ...
    if (!decision)
        return { evidence: [] };
    const statement = [decision.decision, decision.rationale].filter((value) => Boolean(value?.trim())).join('\n');
    if (!statement)
        return { evidence: [] };
    const rationale = {
...
function scopeToProject(graph, activeEntities, activeIds, project, asOf) {
...
function pathToProject(entityId, projectId, edges) {
    if (entityId === projectId)
        return [];
    const queue = [{ id: entityId, path: [] }];
    const visited = new Set([entityId]);
    while (queue.length > 0) {
        const current = queue.shift();
        for (const edge of edges) {
            const relation = getCanonicalRelation(edge.relation);
            if (relation.scopeTraversal === 'none' || relation.traversalDirection === 'none')
                continue;
...
function resolveProject(entities, requested) {
    if (!requested)
        return undefined;
...
function entityEmbeddingText(entity) { return entitySearchFields(entity).filter(Boolean).join('\n'); }
function entitySearchText(entity) { return [entity.summary ?? entity.name, metadataText(entity.metadata)].filter(Boolean).join('\n'); }
function metadataText(metadata) { return metadata ? JSON.stringify(metadata) : ''; }
...
function normalize(value) { return value.normalize('NFKC').replace(/\s+/g, '').toLowerCase(); }
function rankStates(states) { return [...states].sort(compareStates); }
function compareStates(left, right) { return right.score - left.score || left.entity.id.localeCompare(right.entity.id); }
function unique(values) { return [...new Set(values)]; }
function migrationResponse(input) {
