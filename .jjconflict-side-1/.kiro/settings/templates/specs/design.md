...
### brainbase Layer Structure (CLAUDE.md 1.4)
```
UI Components (View)
    ↓ EventBus
...
- Tests: `tests/domain/{domain}/` or `tests/unit/`
...
##### Service Interface (brainbase DI Container - CLAUDE.md 1.3)
```javascript
// Service class definition (public/modules/domain/{domain}/{domain}-service.js)
export class {{Domain}}Service {
  ...
  async get{{Domain}}s(filters) {
    const items = await this.repository.fetch{{Domain}}s();
    const filtered = this._applyFilters(items, filters);
    this.store.setState({ {{domain}}: filtered });
    this.eventBus.emit(EVENTS.{{DOMAIN}}_LOADED, { items: filtered });
    return filtered;
  }

  async update{{Domain}}(id, changes) {
    await this.repository.update{{Domain}}(id, changes);
    this.eventBus.emit(EVENTS.{{DOMAIN}}_UPDATED, { id, changes });
  }
...
// DI Container registration (public/app.js or initialization)
container.register('{{domain}}Repository', () => new {{Domain}}Repository());
container.register('{{domain}}Service', () => new {{Domain}}Service({
  repository: container.get('{{domain}}Repository'),
...
##### Event Contract (brainbase EventBus - CLAUDE.md 1.1)
```javascript
// Event definitions (add to public/modules/core/event-bus.js EVENTS constant)
export const EVENTS = {
  // ... existing events
  {{DOMAIN}}_LOADED: '{{domain}}:loaded',
  {{DOMAIN}}_UPDATED: '{{domain}}:updated',
  {{DOMAIN}}_DELETED: '{{domain}}:deleted',
};

// Event emission pattern
eventBus.emit(EVENTS.{{DOMAIN}}_LOADED, { items: [] });

// Event subscription pattern (sync)
eventBus.on(EVENTS.{{DOMAIN}}_UPDATED, (event) => {
  const { id, changes } = event.detail;
  // Handle update
});

// Event subscription pattern (async)
eventBus.onAsync(EVENTS.{{DOMAIN}}_LOADED, async (event) => {
  await processData(event.detail);
});
```
- Published events:
- Subscribed events:
...
##### State Management (brainbase Reactive Store - CLAUDE.md 1.2)
```javascript
// Store extension (add to public/modules/core/store.js or service initialization)
appStore.setState({
  {{domain}}: [],           // Data array
  {{domain}}Filters: {},    // Filter settings
  current{{Domain}}Id: null // Current selection
});

// Store subscription pattern
const unsubscribe = appStore.subscribe((change) => {
  if (change.key === '{{domain}}') {
    render(); // UI auto-update
  }
});

// State update pattern (in Service)
this.store.setState({ {{domain}}: updatedData });
```
- State model:
