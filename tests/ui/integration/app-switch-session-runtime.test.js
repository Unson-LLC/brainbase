...
describe('app switchSession runtime handling', () => {
  ...
  it('xterm snapshot時_terminal headerとsnapshot panelを表示する', async () => {
    appStore.setState({
      currentSessionId: 'session-1',
      sessions: [{
        id: 'session-1',
    ...
    app._terminalSnapshotCache.set('session-1', {
      text: 'last pane snapshot',
      capturedAt: '2026-03-15T10:20:00.000Z'
    });
    app.reconnectManager = {
      wsConnected: true,
      isReconnecting: false,
    ...
    expect(app.terminalTransportPillEl.textContent).toBe('xterm');
    expect(app.terminalSnapshotMetaEl.classList.contains('hidden')).toBe(false);
    expect(app.terminalReconnectBtn.classList.contains('hidden')).toBe(false);
    expect(app.terminalSnapshotPanelEl.classList.contains('hidden')).toBe(false);
    expect(app.terminalSnapshotContentEl.textContent).toContain('last pane snapshot');
  });

  it('forceTtyd指定時_xtermを使わずiframe fallbackへ切り替える', async () => {
    app.reconnectManager = { setCurrentSession: vi.fn(), terminalAccess: null };
    app._shouldUseXtermTransport = vi.fn(() => true);
    app.terminalTransportClient = { disconnect: vi.fn(), hide: vi.fn(), destroy: vi.fn() };
