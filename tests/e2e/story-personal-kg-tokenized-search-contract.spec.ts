...
test('story-personal-kg-tokenized-search ac:3 ac:4 ac:5 C-003 C-004 S-002 single token preserves filters and does not broaden to OR', async () => {
  ...
  expect(search?.sql, 'ac:5 owner/redaction/rejected filters remain in the read path').toContain("owner_person_id = $1 AND visibility = 'owner' AND redaction_status = 'none' AND promotion_status <> 'rejected'");
