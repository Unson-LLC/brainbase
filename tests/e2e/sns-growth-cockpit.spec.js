...
test.describe('SNS Growth Cockpit mobile workspace mode', () => {
    const { defaultBrowserType: _defaultBrowserType, ...iphone12Pro } = devices['iPhone 12 Pro'];
    test.use(iphone12Pro);

    test('review card tap keeps SNS detail open instead of revealing the terminal', async ({ page, request }) => {
        const lane = `mobile_review_guard_e2e_${Date.now()}`;
        const accountId = `acc_x_sato_${lane}`;
        const headers = await csrfHeaders(request, lane);
        const date = todayJst();
        const body = `モバイルレビューでターミナルを開かない ${Date.now()}`;
        ...
        const pack = await request.post(`${BASE_URL}/api/sns-growth/review-pack`, {
            ...
            data: {
                ...
                drafts: [{
                    ...
                    lane,
                    body,
                    source_url: 'https://x.com/example/status/mobile-review-guard',
                    persona_brain: { target_person: 'AI導入を任されたPM' },
                    quality_gate: { status: 'pass' }
                }]
            }
