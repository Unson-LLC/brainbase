...
test.describe('story-session-switch-connect-hang', () => {
    test('AC: a superseded pending connect rejects instead of hanging', async ({ page }) => {
        ...
        expect(outcome.res, 'A superseded pending connect rejects deterministically as superseded instead of hanging, so the awaiting switch reaches the snapshot fallback instead of freezing forever').not.toBe('HUNG');
