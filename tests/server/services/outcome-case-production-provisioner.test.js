...
function createService({ capabilityStatus = null, authorityPresent = false, actorExists = true } = {}) {
    ...
    const client = {
        async query(sql, parameters = []) {
            ...
            if (compact.includes('FROM graph_entities') && compact.includes("entity_type = 'person'")) {
                if (parameters[0] === 'per_unknown' && !actorExists) return { rows: [] };
...
function createConcurrentService() {
    const state = { capabilityStatus: null, authorityPresent: false };
    const lockKeys = [];
    let lockTail = Promise.resolve();
    const createRaci = vi.fn(async () => {
        state.authorityPresent = true;
        return { raci_id: 'rac_outcome_case_close', event_id: 'evt_outcome_case_close' };
    });
    const service = {
        createRaci,
        withAccessContext: vi.fn(async (_access, handler) => {
            let releaseLock = null;
            const client = {
                async query(sql, parameters = []) {
                    const compact = String(sql).replace(/\s+/gu, ' ').trim();
                    if (compact.includes('pg_advisory_xact_lock')) {
                        lockKeys.push(parameters[0]);
    ...
    return { service, createRaci, lockKeys };
}
...
describe('OutcomeCase production control-plane provisioning', () => {
    ...
    it('rejects an apply actor that is not an existing canonical person', async () => {
        const { service } = createService({ actorExists: false });

        await expect(provisionOutcomeCaseControlPlane({
            infoSSOTService: service,
            actorPersonId: 'per_unknown',
            commit: true
        })).rejects.toThrow(expect.objectContaining({ code: 'PROVISIONING_ACTOR_NOT_FOUND' }));
        expect(service.createRaci).not.toHaveBeenCalled();
    });

    it('serializes concurrent applies and re-reads after the fixed control-plane lock', async () => {
        const { service, createRaci, lockKeys } = createConcurrentService();

        await Promise.all([
            provisionOutcomeCaseControlPlane({
                infoSSOTService: service,
                actorPersonId: 'per_operator_one',
                commit: true
            }),
            provisionOutcomeCaseControlPlane({
                infoSSOTService: service,
                actorPersonId: 'per_operator_two',
                commit: true
            })
        ]);

        expect(lockKeys).toEqual([
            'outcome-case-control-plane:unson:prj_01KGCS8CAJKKDWACPNK1E5WX8H',
            'outcome-case-control-plane:unson:prj_01KGCS8CAJKKDWACPNK1E5WX8H'
        ]);
        expect(createRaci).toHaveBeenCalledTimes(1);
    });
