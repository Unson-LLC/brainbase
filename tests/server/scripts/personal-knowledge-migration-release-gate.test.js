...
describe('personal knowledge migration release gate', () => {
  ...
  it.each([
    ['approved mapped to owner_rejected', { pending_owner_approval: 1, owner_rejected: 2, rejected: 1 }],
    ['rejected mapped to org_accepted', { pending_owner_approval: 1, approved: 2, org_accepted: 1 }],
    ['legacy terminal mappings mixed', { pending_owner_approval: 1, org_accepted: 1, owner_rejected: 2 }]
  ])('rejects %s', (_label, status_counts) => {
    const legacyBefore = {
      ...before,
      status_counts: { pending_owner_approval: 1, approved: 2, rejected: 1 },
      target_request_ids: [],
    ...
    expect(() => assertPostflight(legacyBefore, { ...after, status_counts, total: 4 }, [], rls)).toThrow('status');
  });
