...
describe('tenant runtime API', () => {
    ...
    it('D-003/D-005: authoritative revision検証とopaque credential leaseをAPI化する', async () => {
        const revision = await request(createApp()).post('/api/v1/runtime/workspace-connections:validate-revision').set('authorization', 'Bearer service-test').send({ tenant_id: 'ten_01ARZ3NDEKTSV4RRFFQ69G5FAV', connection_id: 'wsc_01ARZ3NDEKTSV4RRFFQ69G5FAV', expected_connection_revision: 1 });
        const lease = await request(createApp()).post('/api/v1/runtime/credential-leases').set('authorization', 'Bearer service-test').send({ credential_ref: 'credref:opaque', operation_id: 'op_01ARZ3NDEKTSV4RRFFQ69G5FAV', audience: 'mana-runtime', ttl_seconds: 60 });
    ...
    it('AC-304: failureをapplication/problem+jsonと機械判定可能なfault_domainで返す', async () => {
        ...
        const response = await request(app).post('/api/v1/runtime/workspace-connections:validate-revision').set('authorization', 'Bearer service-test').send({});
