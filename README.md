...
Direct local `release:publish` is rejected because it would bypass the package-wide Actions concurrency queue. Local `release:plan`, `release:validate`, and `release:verify` remain available for credential-free diagnosis; all registry mutation goes through the workflow above.
