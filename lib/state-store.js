...
export class StateStore {
    ...
    async _reloadFromFile() {
        try {
            ...
            logger.error('[StateStore] Failed to reload:', err.message);
        }
    }
    ...
    async _tryLoadState(filePath) {
        try {
            ...
            logger.warn(`[StateStore] Failed to load ${filePath}: ${e.code || e.message}`);
            return null;
        }
    }
    ...
    async cleanup() {
        ...
        try {
            ...
            logger.error('[StateStore] Cleanup: failed to persist state:', err.message);
        }
        if (this._watcher) {
    ...
    async cleanupOrphanedTmpFiles() {
        try {
            ...
            logger.warn('[StateStore] Failed to clean up tmp files:', err.message);
        }
    }
}
