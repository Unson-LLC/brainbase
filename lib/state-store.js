...
export class StateStore {
    ...
    _normalizeSession(nextSession, previousSession = null) {
        ...
        const durablePath = [
            ...
            previousKnownGood
        ].find((candidate) => candidate && !this._isEphemeralWorkspacePath(candidate)) || null;
    ...
    _normalizeSessions(sessions, previousSessions = []) {
        ...
        return (sessions || [])
            .map((session) => this._normalizeSession(session, previousMap.get(session.id) || null))
            .filter(Boolean);
    }
    ...
    async deleteSession(sessionId) {
        return this.mutateSessions((sessions) => sessions.filter((session) => session.id !== sessionId));
    }
