...
import { createAtomicRunner } from './utils/atomic-runner.js';
...
export class TaskFileManager {
    ...
    constructor(rootPath) {
        ...
        this.runAtomic = createAtomicRunner({ label: 'TaskFileManager atomic operation' });
    }

    _normalizeTask(sourceTask, overrides = {}) {
        if (!sourceTask) {
            return null;
        }
        ...
        return {
            id: overrides.id ?? sourceTask.id ?? metadata.ID,
            name: overrides.name ?? sourceTask.name,
            status: overrides.status ?? sourceTask.status ?? metadata.Status ?? 'todo',
            priority: overrides.priority ?? metadata.Priority ?? null,
            due: overrides.due ?? metadata.Due ?? null,
            parentId: overrides.parentId ?? metadata.Parent ?? null,
            description: overrides.description ?? metadata.Description ?? null,
            owner: overrides.owner ?? metadata.Owner ?? null
        };
    }
    ...
    async completeTask(projectId, taskId) {
        return this.runAtomic(async () => {
            ...
            const completedTask = this._normalizeTask(removedTask, { status: 'done' });

            // Read done.md and prepend task (newest first)
            const doneContent = await this.readFileSafe(paths.donePath);
            const newDoneContent = this.parser.prependTask(doneContent, completedTask);
    ...
    async restoreTask(projectId, taskId, newStatus = 'todo') {
        return this.runAtomic(async () => {
            ...
            const activeTask = this._normalizeTask(removedTask, { status: newStatus });
    ...
    async restoreOrUpdateTask(projectId, taskId, newStatus) {
        return this.runAtomic(async () => {
            ...
            if (found) {
                ...
                const updatedTask = this._normalizeTask(found.task, { status: newStatus });
            ...
            if (found) {
                ...
                const activeTask = this._normalizeTask(found.task, { status: newStatus });
     ...
     * Create a new task
     * @param {string} projectId - Project ID
     * @param {Object} taskData - Task data { name, priority?, due?, parentId?, description?, owner? }
    ...
    async updateTask(projectId, taskId, updates) {
        ...
        // Handle project change (move task to different project)
        if (updates.project && updates.project !== projectId) {
            return this.moveTask(projectId, updates.project, taskId, updates);
        }
        ...
        return this.runAtomic(async () => {
            ...
            const normalizedTask = this._normalizeTask(found.task);

            const updatedTask = {
                ...normalizedTask,
                name: updates.name ?? updates.title ?? normalizedTask.name,
                priority: updates.priority ?? normalizedTask.priority,
                due: updates.due !== undefined
                    ? updates.due
                    : (updates.deadline !== undefined ? updates.deadline : normalizedTask.due),
                description: updates.description !== undefined ? updates.description : normalizedTask.description,
                owner: updates.owner !== undefined ? updates.owner : normalizedTask.owner,
                status: updates.status !== undefined ? updates.status : normalizedTask.status
            };
     ...
     * @returns {Promise<boolean>} - Success status
     */
    async moveTask(fromProjectId, toProjectId, taskId, updates = {}) {
        return this.runAtomic(async () => {
            const fromPaths = this.getProjectPaths(fromProjectId);
            const toPaths = this.getProjectPaths(toProjectId);

            // Ensure destination project directory exists
            await this.ensureProjectDir(toProjectId);

            // Find and remove task from source project
            let content = await this.readFileSafe(fromPaths.tasksPath);
            let found = this.parser.findTaskById(content, taskId);
            let sourceFilePath = fromPaths.tasksPath;
            let destFilePath = toPaths.tasksPath;

            // If not found in tasks.md, try done.md
            if (!found) {
                content = await this.readFileSafe(fromPaths.donePath);
                found = this.parser.findTaskById(content, taskId);
                sourceFilePath = fromPaths.donePath;
                destFilePath = toPaths.donePath;
            }
            ...
            const movedTask = {
                ...
                description: updates.description !== undefined ? updates.description : normalizedTask.description,
                priority: updates.priority ?? normalizedTask.priority,
                owner: newOwner !== undefined ? newOwner : normalizedTask.owner,
                status: normalizedTask.status === 'done' ? 'done' : 'todo'
            };

            // Read destination file and append task
            const destContent = await this.readFileSafe(destFilePath);
            const newDestContent = this.parser.appendTask(destContent, movedTask);

            // Write both files
            await fs.writeFile(sourceFilePath, contentWithoutTask, 'utf-8');
            await fs.writeFile(destFilePath, newDestContent, 'utf-8');
