...
/** @typedef {{ tasksPath: string, donePath: string, dirPath: string }} ProjectPaths */
/** @typedef {{ name?: string, title?: string, priority?: string, due?: string | null, deadline?: string | null, parentId?: string | null, description?: string | null, owner?: string | null, project?: string, status?: string }} TaskUpdatePayload */
...
export class TaskFileManager {
     ...
     * @returns {ProjectPaths} - { tasksPath, donePath, dirPath }
     */
    ...
    async completeTask(projectId, taskId) {
        return this.runAtomic(async () => {
            ...
            const completedTask = {
                ...
                description: removedTask.metadata?.Description,
                owner: removedTask.metadata?.Owner
            };
    ...
    async restoreOrUpdateTask(projectId, taskId, newStatus) {
        return this.runAtomic(async () => {
            ...
            const updatedTask = {
                ...
                owner: found.task.metadata?.Owner
            };
     ...
     * Create a new task
     * @param {string} projectId - Project ID
     * @param {TaskUpdatePayload} taskData - Task data { name, priority?, due?, parentId?, description?, owner? }
     * @returns {Promise<import('./kiro-task-parser.js').FinalKiroTask>} - Created task object
     */
     ...
     * @param {TaskUpdatePayload} updates - Updates { name?, priority?, due?, status? }
     * @returns {Promise<boolean>} - Success status
     */
    async updateTask(projectId, taskId, updates) {
        ...
        return this.runAtomic(async () => {
            ...
            // Create updated task (support both name/title and due/deadline)
            const newName = updates.name || updates.title || found.task.name;
            const newDue = updates.due !== undefined ? updates.due :
                          (updates.deadline !== undefined ? updates.deadline : found.task.metadata?.Due);
            const newDescription = updates.description !== undefined ? updates.description : found.task.metadata?.Description;
            const newOwner = updates.owner !== undefined ? updates.owner : found.task.metadata?.Owner;
     ...
     * @param {TaskUpdatePayload} [updates] - Additional updates to apply
     * @returns {Promise<boolean>} - Success status
     */
    async moveTask(fromProjectId, toProjectId, taskId, updates = {}) {
        return this.runAtomic(async () => {
            ...
            // Create updated task with new project
            const newName = updates.name || updates.title || found.task.name;
            const newDue = updates.due !== undefined ? updates.due :
                          (updates.deadline !== undefined ? updates.deadline : found.task.metadata?.Due);
            const newDescription = updates.description !== undefined ? updates.description : found.task.metadata?.Description;
            ...
            const movedTask = {
                id: taskId,
                name: newName,
                status: found.task.status === 'done' ? 'done' : 'todo',
                priority: updates.priority || found.task.metadata?.Priority,
                due: newDue,
                parentId: found.task.metadata?.Parent,
                description: newDescription,
     ...
     * @param {string} taskId - Task ID
     * @returns {Promise<{ projectId: string, fileType: 'tasks' | 'done', task: import('./kiro-task-parser.js').RawKiroTask } | null>} - { projectId, fileType, task } or null
     */
...
Object.assign(TaskFileManager.prototype, AtomicMixin);
