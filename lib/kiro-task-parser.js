...
export class KiroTaskParser {
    ...
    _finalizeTask(task, projectId) {
        // Use Status metadata if present (for in_progress etc), otherwise use checkbox status
     ...
     */
    serializeTask(task) {
        // Checkbox: [x] for done, [ ] for all other statuses
        ...
        // Status metadata: save if not 'done' or 'todo' (e.g., 'in_progress')
        if (task.status && task.status !== 'done' && task.status !== 'todo') {
            lines.push(`  - _Status: ${task.status}_`);
        }
