...
/** @typedef {{ id?: string, channel?: string, sender?: string, timestamp?: string, status?: string, title?: string, message?: string, slackUrl?: string, date?: string, time?: string }} InboxItem */
/** @typedef {{ title?: string, message?: string, sender?: string, channel?: string, instruction?: unknown, source?: string }} InboxAppendInput */

export class InboxParser {
    /**
     * @param {string} inboxFilePath
     */
    ...
    parseMarkdown(markdown) {
        ...
        for (let i = 1; i < blocks.length; i++) {
            ...
            try {
                ...
                logger.warn('Failed to parse inbox block:', { error: e });
            }
        }
    ...
    async markAsDone(itemId) {
        return this.runAtomic(async () => {
            try {
                ...
                logger.error('Error marking item as done:', { error });
                return false;
            }
        });
    }
    ...
    async markAllAsDone() {
        return this.runAtomic(async () => {
            try {
                ...
                logger.error('Error marking all items as done:', { error });
                return false;
            }
        });
    }
    ...
    /**
     * @param {InboxAppendInput} input
     * @param {Date} now
     * @returns {Promise<InboxItem>}
     */
    async appendItem(input, now = new Date()) {
        return this.runAtomic(async () => {
            const item = normalizeAppendInput(input, now);
            const existing = await readInboxFile(this.inboxFilePath);
            const prefix = existing.trim()
                ? `${existing.replace(/\s*$/, '')}\n\n`
                : '# Pending Inbox Items\n\n<!-- AI PMが自動更新。Claude Code起動時に確認・対応を提案 -->\n\n';
            await fs.writeFile(this.inboxFilePath, `${prefix}${formatInboxItemMarkdown(item, input.instruction)}\n`, 'utf-8');
            return item;
        });
    }
}

Object.assign(InboxParser.prototype, AtomicMixin);
...
function normalizeAppendInput(input, now) {
    const timestamp = now.toISOString();
    const idSuffix = now.getTime().toString();
    const title = cleanText(input.title || 'Daily Ops instruction');
    const message = cleanText(input.message || title);
    return {
        id: `INBOX-DAILY-${idSuffix}`,
        channel: cleanField(input.channel || 'daily-ops'),
        sender: cleanField(input.sender || 'Daily Ops Report'),
        timestamp,
        status: 'pending',
        title,
        message,
        date: now.toLocaleDateString('ja-JP', { month: 'numeric', day: 'numeric' }),
        time: now.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit', hour12: false })
    };
}

function formatInboxItemMarkdown(item, instruction) {
    const actionJson = instruction == null
        ? ''
        : `\n\n**Action JSON**\n\n\`\`\`json\n${JSON.stringify(instruction, null, 2)}\n\`\`\``;
    return `---\nid: ${item.id}\nchannel: ${item.channel}\nsender: ${item.sender}\ntimestamp: ${item.timestamp}\nstatus: pending\n---\n### ${item.time} | #${item.channel} | ${item.sender}\n\n${item.message}${actionJson}`;
}
