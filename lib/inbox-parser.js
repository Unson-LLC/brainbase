    ...
    async markAsDone(itemId) {
        return this.runAtomic(async () => {
            try {
                ...
                // INBOX-1766718363259249 形式からタイムスタンプ部分を抽出
                const timestampMatch = itemId.match(/INBOX-(\d+)/);
                if (!timestampMatch) {
