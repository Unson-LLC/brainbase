...
export class ConfigParser {
    ...
    async _loadYamlFile(filePath, nameForError = 'YAML file') {
        try {
            const content = await fs.readFile(filePath, 'utf-8');
            return yaml.load(content) || {};
        } catch (err) {
            console.error(`Failed to load ${nameForError}:`, err.message);
            return null;
        }
    }

    async _loadSlackYaml(filename) {
    ...
    _expandProjectLocalPaths(projects = []) {
    ...
    async getProjects() {
        ...
        if (!data) {
            return { root: this.workspaceRoot, projects: [] };
        }

        const projects = this._expandProjectLocalPaths(data.projects || []);
        return {
    ...
    async getNocoDBMappings() {
        ...
        return (data.projects || [])
            ...
            .map(p => ({
                ...
                legacy_base_id: p.nocodb.base_id,
                nocodb_project_id: p.nocodb.project_id || '',
