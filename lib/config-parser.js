...
// @ts-ignore js-yaml does not ship types in this repo setup
import yaml from 'js-yaml';
import { logger } from '../server/utils/logger.js';

/** @typedef {Record<string, any>} AnyRecord */
/** @typedef {AnyRecord & { id?: string, name?: string, catalog_version?: number, emoji?: string, archived?: boolean, local?: AnyRecord, github?: AnyRecord, airtable?: AnyRecord, nocodb?: AnyRecord }} ProjectConfig */
/** @typedef {{ projects?: ProjectConfig[], plugins?: AnyRecord, organizations?: AnyRecord[], dependencies?: AnyRecord, notifications?: AnyRecord }} RootConfig */
...
export class ConfigParser {
     ...
     * @param {{ catalogMode?: 'required' | 'disabled' }} [options]
     */
    constructor(codexPath, configPath, brainbaseRoot = process.env.WORKSPACE_ROOT || '/path/to/workspace', projectsRoot = null, options = {}) {
        this.codexPath = codexPath;
        this.configPath = configPath;
        this.brainbaseRoot = brainbaseRoot;
        this.projectsRoot = projectsRoot || path.join(brainbaseRoot, 'projects');
        this.catalogMode = options.catalogMode || 'required';
        this._configSourceStatus = this.catalogMode === 'disabled' ? 'not_applicable' : 'not_loaded';
     ...
     * @returns {Promise<T>}
     */
    async _loadYamlFile(filePath, defaultValue, label) {
        try {
            const content = await fs.readFile(filePath, 'utf-8');
            const data = /** @type {T | undefined} */ (yaml.load(content));
            return data ?? defaultValue;
        } catch (err) {
            const target = label || filePath;
            const error = /** @type {NodeJS.ErrnoException} */ (err);
            if (error?.code === 'ENOENT' && filePath.startsWith(this.slackDir)) {
                logger.debug(`Optional config missing: ${target}`, { filePath });
                return defaultValue;
            }
            logger.error(`Failed to load ${target}:`, { error: error.message || String(error) });
            return defaultValue;
        }
    }
     ...
     * @returns {Promise<T>}
     */
    async _loadSlackYaml(filename, defaultValue) {
        return this._loadYamlFile(
            path.join(this.slackDir, filename),
            defaultValue,
            filename
        );
    }
    ...
    async _loadConfig() {
        ...
        if (this.catalogMode === 'disabled') {
            this._configSourceStatus = 'not_applicable';
            this._configCache = {};
            logger.debug('Project catalog is disabled for this runtime', {
        ...
        if (!this._configPromise) {
            this._configPromise = fs.readFile(this.configPath, 'utf-8')
                .then((content) => {
                    const data = /** @type {RootConfig | undefined} */ (yaml.load(content));
                    this._configSourceStatus = 'loaded';
                    return data ?? {};
                })
                .catch((err) => {
                    const error = /** @type {NodeJS.ErrnoException} */ (err);
                    this._configSourceStatus = error?.code === 'ENOENT' ? 'missing' : 'invalid';
    ...
    /**
     * SPEC-settings-phase0-guards INV-4: ConfigService.write 後に呼ぶことで stale cache を回避
     */
    invalidateCache() {
        this._configCache = null;
        this._configPromise = null;
     ...
     */
    async getWorkspaces() {
        const data = /** @type {AnyRecord} */ (await this._loadSlackYaml('workspaces.yml', /** @type {AnyRecord} */ ({})));
     ...
     */
    async getChannels() {
        const data = /** @type {AnyRecord} */ (await this._loadSlackYaml('channels.yml', /** @type {AnyRecord} */ ({})));
     ...
     */
    async getMembers() {
        const data = /** @type {AnyRecord} */ (await this._loadSlackYaml('members.yml', /** @type {AnyRecord} */ ({})));
        return data?.members || [];
    }
    ...
    async getProjects() {
        ...
        const projects = this._expandProjectPaths(data.projects || []).map(project => {
            // Legacyの未認証設定APIへProject Profileのtenant・人物・接続意図を露出させない。
            const {
     ...
     * @returns {Promise<any>} { enabled: string[] } - 有効なプラグインID一覧
     */
    async getPlugins() {
        const data = await this._loadConfig();
        const plugins = data.plugins || {};

        if (!plugins.enabled && !plugins.disabled) {
    ...
    async checkIntegrity() {
        ...
        /** @type {any[]} */
        const issues = [];
        if (this._configSourceStatus !== 'loaded') {
            issues.push({
                type: 'config_source_unavailable',
                severity: 'error',
                message: `Required project catalog is ${this._configSourceStatus}: ${this.configPath}`,
                source: 'config.yml'
            });
        }
        ...
        return {
            ...
            source: {
                status: this._configSourceStatus,
                path: this.configPath,
                mode: this.catalogMode
            },
            issues,
