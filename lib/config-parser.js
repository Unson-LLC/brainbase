...
export class ConfigParser {
    ...
    async getNocoDBMappings() {
        try {
            ...
            console.error('Failed to load NocoDB mappings:', err.message);
            return [];
        }
    }
    ...
    async getPlugins() {
        try {
            ...
            return {
                ...
                // 将来の拡張用: disabled, settings等を追加可能
            };
        } catch (err) {
            console.error('Failed to load plugins config:', err.message);
            // デフォルト: すべてのプラグインを有効
            return {
                enabled: ['bb-dashboard', 'bb-focus-task', 'bb-next-tasks', 'bb-schedule', 'bb-inbox']
            };
        }
    }
