...
export class ConfigParser {
    ...
    async _loadYamlFile(filePath, defaultValue, label) {
        try {
            ...
            if (err?.code === 'ENOENT' && filePath.startsWith(this.slackDir)) {
    ...
    async _loadConfig() {
        ...
        if (!this._configPromise) {
            this._configPromise = this._loadYamlFile(this.configPath, {}, 'config.yml');
        }
     ...
     */
    async getWorkspaces() {
        const data = await this._loadSlackYaml('workspaces.yml', {});
     ...
     */
    async getChannels() {
        const data = await this._loadSlackYaml('channels.yml', {});
     ...
     */
    async getMembers() {
        const data = await this._loadSlackYaml('members.yml', {});
        return data?.members || [];
    }
