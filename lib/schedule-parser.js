...
export class ScheduleParser {
    ...
    async getSchedule(date) {
        const { events: googleEvents, error } = await this._loadGoogleCalendarEvents(date);
    ...
    async _loadGoogleCalendarEvents(date) {
        ...
        try {
            return {
                events: await this.googleCalendarService.listEventsForDate(date),
                error: null
            };
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            logger.warn('[ScheduleParser] Failed to load Google Calendar events:', message);
