...
/** @typedef {{ projectId: string, tasksPath: string, donePath: string, dirPath: string }} ProjectPaths */
...
export class TaskDirectoryScanner {
     ...
     * @returns {Promise<ProjectPaths[]>} - Array of { projectId, tasksPath, donePath, dirPath }
     */
    async scanProjects() {
        try {
     ...
     * @returns {Promise<import('./kiro-task-parser.js').FinalKiroTask[]>} - Array of task objects
     */
    async getActiveTasks() {
        const projects = await this.scanProjects();
        /** @type {import('./kiro-task-parser.js').FinalKiroTask[]} */
        const allTasks = [];
        ...
        for (const project of projects) {
            try {
                ...
                if (!(error instanceof Error) || /** @type {NodeJS.ErrnoException} */ (error).code !== 'ENOENT') {
                    logger.error(`Error reading tasks for ${project.projectId}:`, error);
                }
     ...
     * @returns {Promise<import('./kiro-task-parser.js').FinalKiroTask[]>} - Array of completed task objects
     */
    async getCompletedTasks() {
        ...
        for (const project of projects) {
            try {
                ...
                if (!(error instanceof Error) || /** @type {NodeJS.ErrnoException} */ (error).code !== 'ENOENT') {
                    logger.error(`Error reading done tasks for ${project.projectId}:`, error);
                }
     ...
     * @returns {Promise<{ task: import('./kiro-task-parser.js').RawKiroTask, projectId: string, filePath: string, fileType: 'tasks' | 'done' } | null>} - { task, projectId, filePath, fileType } or null
     */
    async findTaskById(taskId) {
        ...
        for (const project of projects) {
            ...
            try {
                ...
                if (!(error instanceof Error) || /** @type {NodeJS.ErrnoException} */ (error).code !== 'ENOENT') {
                    logger.error(`Error searching tasks for ${project.projectId}:`, error);
                }
            }
            ...
            try {
                ...
                if (!(error instanceof Error) || /** @type {NodeJS.ErrnoException} */ (error).code !== 'ENOENT') {
                    logger.error(`Error searching done tasks for ${project.projectId}:`, error);
                }
            }
        }
