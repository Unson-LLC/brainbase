// @ts-check
import { KIRO_METADATA_REGEX } from './kiro-task-parser.js';
...
import {
    ...
    isGoogleCalendarDuplicate
} from './google-calendar-utils.js';

/** @typedef {{ ID?: string, Source?: string, CalendarId?: string }} ScheduleMetadata */
/** @typedef {{ start: string, end: string | null, title: string, completed: boolean, lineNumber?: number | null, metadata: ScheduleMetadata, id?: string, allDay?: boolean, source?: string | null, calendarId?: string | null }} RawScheduleEvent */
/** @typedef {{ id: string, start: string, end: string | null, title: string, source: string | null, calendarId: string | null, completed: boolean, lineNumber: number | null | undefined, allDay?: boolean }} FinalScheduleEvent */
...
export class KiroScheduleParser {
     ...
     * @returns {RawScheduleEvent|null} - Parsed event info or null if not an event
     */
    parseEvent(line) {
     ...
     * @param {string} line - Line to parse
     * @returns {ScheduleMetadata|null} - Parsed metadata or null if not metadata
     */
    parseMetadata(line) {
        ...
        return /** @type {ScheduleMetadata} */ ({ [key]: value.trim() });
    }

    /**
     * Parse file content into events
     * @param {string} content - File content
     * @returns {FinalScheduleEvent[]} - Array of event objects
     */
    parseFile(content) {
        ...
        /** @type {FinalScheduleEvent[]} */
        const events = [];
        /** @type {RawScheduleEvent | null} */
     ...
     * @param {RawScheduleEvent} event - Raw event object
     * @returns {FinalScheduleEvent} - Finalized event object
     */
     ...
     * @param {FinalScheduleEvent} event - Event object
     ...
     * @returns {{ startLine: number | null, endLine: number, event: FinalScheduleEvent } | null} - { startLine, endLine, event } or null
     */
     ...
     * @returns {{ content: string, removedEvent: FinalScheduleEvent | null }} - { content: string, removedEvent: Object|null }
     */
     ...
     * @param {string} content - File content
     * @param {FinalScheduleEvent} event - Event object to append
     ...
     * @param {FinalScheduleEvent[]} existingEvents - Existing events
     * @param {FinalScheduleEvent} newEvent - New event to check
     * @returns {boolean} - True if duplicate
     */
    isDuplicate(existingEvents, newEvent) {
        return isGoogleCalendarDuplicate(existingEvents, newEvent);
    }
}
