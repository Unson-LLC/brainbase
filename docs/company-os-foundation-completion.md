...
既存の8件は、元26 StoryのOSS 7件と、別枠のsidecar補助1件で構成する。今回、元26 Storyの `story-company-os-evaluation-v1`（07）、`story-company-os-problem-candidates-v1`（08）、`story-company-os-problem-selection-v1`（09）、`story-company-os-execution-authority-v1`（11）、`story-company-os-durable-waits-v1`（13）、`story-company-os-learning-adoption-v1`（14）、`story-company-os-objective-editor-v1`（16）、`story-company-os-judgment-view-v1`（17）、`story-company-os-decision-adapter-v1`（19）がmerge済みとなり、元26 Storyの完了数は16件（01, 02, 03, 04, 05, 06, 07, 08, 09, 10, 11, 13, 14, 16, 17, 19）になった。sidecar補助は元26 Storyの件数に含めない。

完了済みのOSS基盤は、目的・制約・問題候補・選択・評価・待機・学習採用・目的編集・判断表示・Decision参照を版付きの共通契約で扱い、SSOT sidecarとHTTP実行権限portを介して接続する。下表は各機能の実装、レビュー、CI、mergeの状態を集約し、未mergeの実装は完了数へ算入しない。
...
| 元26 Story・実装済み・未完了 | `story-company-os-effect-reconciliation-v1`（12、owner: `mana-runtime`）「成否不明の外部実行を重複させず照合して回復できる」 | [PR #1392](https://github.com/Unson-LLC/mana-runtime/pull/1392) head `f69a6747`（main競合解消、67 tests・4 profiles pass）、review delta進行中。未mergeのため完了件数へは算入しない |
...
| 元26 Story・実装済み・未完了 | `story-company-os-impact-review-v1`（15、owner: `brainbase`）「前提の変更で影響する計画だけを再判断に戻せる」 | [PR #543](https://github.com/Unson-LLC/brainbase/pull/543) head `46c85fa` / [CI 35858655809](https://github.com/Unson-LLC/brainbase/actions/runs/35858655809) success、8 blockers修正中。未mergeのため完了件数へは算入しない |
...
| 元26 Story・実装済み・未完了 | `story-company-os-knowledge-adapter-v1`（20、owner: `brainbase`）「既存の知識候補と承認から採用判断の条件を辿れる」 | [PR #544](https://github.com/Unson-LLC/brainbase/pull/544) head `fe9e769`（`07d1fff`、exact adoption locator追加）、review（foundation store）進行中。未mergeのため完了件数へは算入しない |
| 元26 Story・実装済み・未完了 | `story-company-os-receipt-adapter-v1`（21、owner: `brainbase`）「既存の実行記録と成果記録を判断へ接続して区別できる」 | [Story本文](stories/story-company-os-receipt-adapter-v1.md) / [PR #539](https://github.com/Unson-LLC/brainbase/pull/539) head `31072a0` / [CI 35860071330](https://github.com/Unson-LLC/brainbase/actions/runs/35860071330) success、JSON非plain object破損のP1修正中。未mergeのため完了件数へは算入しない |
...
| 元26 Story・実装済み・未完了 | `story-company-os-org-learning-approval-v1`（26、owner: `brainbase-organization`）「モデルや目的の改訂を組織の承認で採用できる」 | [PR #109](https://github.com/Unson-LLC/brainbase-organization/pull/109) head `7f959d0`、review（ontology）進行中。未mergeのため完了件数へは算入しない |
...
組織固有・Mana側の未完了Storyや予定を完了へ変更していない。元26 Storyは16/26件がmerge済みで、残り10件は表の状態を維持する。当初計画全体の残り10件へ、今回追加したレビュー追補5件は算入せず、別のplanned登録として扱う。OSS共通基盤の完了は、npm公開、配布repoへの取り込み、組織runtime／Manaへの本番組込みを含まない。
