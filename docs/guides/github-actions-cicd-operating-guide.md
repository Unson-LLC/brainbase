...
|---|---|---|---|---|
| 旧状態管理の退役・障害回復契約 | 対象ファイルのPull Request | 旧state実装の不在、APIの410、旧installerのデータ非変更、Mana統計取得失敗と再試行を検証する。本番接続・配備なし | `.github/workflows/retired-state-boundary.yml` | `[self-hosted, Linux, X64, wsl-linux, nucbox-evo-x2]` |
