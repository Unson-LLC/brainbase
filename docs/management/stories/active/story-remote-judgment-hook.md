...
- [x] AC1: 外部Hostは認証済みHTTP入口から `UserPromptSubmit`、`PostToolUse`、`PostToolUseFailure`、`Stop` を送信できる。
...
- [x] AC5: `PostToolUse`と`PostToolUseFailure`、`Stop`は同じsession/turnのepisodeへ記録され、必要なKnowledge Resolver呼び出しと監査行が欠ける場合はStopをblockする。
