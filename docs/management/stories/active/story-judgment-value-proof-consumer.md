...
- [x] AC-012: runtime 2.4で必要なknowledgeと最終状態が揃っていても、Host監査行が最終assistant回答に欠けた初回Stopは正確な監査ブロックと本文保持指示を返して一度だけ再回答を求める。修復後の実回答を検証できるまでcomplete final receiptを確定しない。
