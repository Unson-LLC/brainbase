...
Program orchestratorは、外部repositoryで発生したmerge/releaseの事実を取り込みつつ、work packageの進捗を根拠なく昇格させない。repo、PR、role、source-lock SHAを一組として照合し、同名の古いopen PRをcanonical producerへ誤採用しない。
...
- [x] AC-003: P0 source-lockのupstream repository、PR、role、SHAがA0 producer deliveryと一致する。
