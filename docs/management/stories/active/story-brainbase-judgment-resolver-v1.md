...
- model生成前に1つの未解決judgment episodeを開始し、Codex modelが最初に`brainbase_resolve_turn`へ意味解釈を渡してTurnContractを確定する。実際に完了した後続のdirect `mcp__brainbase__*` callも`PostToolUse`で0..N件記録する。同一turnの並列callはHostが原子的にjournal commit順へ直列化する。`Stop`は成功した`resolve_turn`証拠とTurnContractの必須capabilityを検証し、回答本文を束縛してcomplete final receiptを1件確定したうえで、保存済み`🧠`行と全`📚`/`⚠️`行をHost自身の`systemMessage`としてjournal commit順に描画する。モデルへ監査行の再現は要求しない。実質的な契約不足は最初の修復可能なStopでのみ`decision:block`となり、なお不完全なactive再Stopは`audit_degraded`へ有限収束する。episode開始event自体がないorphan Stopも完全監査へ偽装せず、警告と本文保持を1回だけ要求した後、非finalの`audit_degraded` receiptへ収束する。identity・integrity矛盾は従来どおり非zeroで失敗する。TurnContract上で追加参照が必須でなく、判断契約以外のBrainbase callが0件なら、その事実をowner監査行として明示する。local file readや別connectorは現行event matcherの対象外とする。
