...
AC-001〜AC-011の機械可読なcoverageは、Taskの`acceptance_coverage`配列を正本とする。各entryは`ac_id`、`assertion_id`、future test path、pre-fix RED assertion、expected result、evidence artifact、release-block conditionを必須とし、11件すべてを1対1で保持する。Taskの`result_contract`はoperationごとにexact shapeを分ける。save/reload成功は単一artifactの`artifact_id`・`run_id`・`record`・`binding`を返し、list成功はそれらを返さず`items`・`count`だけを返す。listの`empty`は`items=[]`・`count=0`、`committed`は検証済みbindingを持つ全committed itemと`count=items.length`である。save/reload失敗は`record=null`、list失敗は`items=null`・`count=null`とし、どちらも副作用をすべてゼロに固定する。`conflict`、`invalid_artifact_id`、`invalid_path`、`path_escape`、`schema_invalid`、`integrity_mismatch`、`binding_missing_or_mismatch`、`non_regular_file`、`cross_filesystem`、`not_found`を安定codeとして列挙する。
