...
- [x] AC-004: A0正本schema/fixtureへ、12 field mappingのID・path・fixture path・type・relation・A0/P0 valueと、12 cross-layer bindingのID・P0/A0左右pathをexact tupleとして固定する。A0 read tupleはingress文脈に限定する。A0の汎用`company_write` authorityは正本へsource-lockするが、P0 dual-authority promotionへ自動昇格させず、P0固有のexact promotion bindingだけを`contract_gap`・`not_collected`・fail closedにする。
