...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-eve-meeting-note-pull-reconciler --stage architecture_spec` を実行し、その後 `vibepro pr prepare . --story-id story-eve-meeting-note-pull-reconciler --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-eve-meeting-note-pull-reconciler/architecture_spec/review-request-regression_risk.md`
...
`vibepro review start . --id story-eve-meeting-note-pull-reconciler --stage architecture_spec --role regression_risk --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-eve-meeting-note-pull-reconciler --stage architecture_spec --role regression_risk --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
