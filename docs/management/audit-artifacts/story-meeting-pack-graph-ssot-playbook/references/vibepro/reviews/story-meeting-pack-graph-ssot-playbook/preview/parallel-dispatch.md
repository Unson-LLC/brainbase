...
- status: stale
- evidence_key: evk_1be8d68e9c0e3821fcba4fe5789dd730
- first_input: false
...
Stale reasons:
- risk_surface_fingerprint: risk_surface_fingerprint changed previous=sha256:e6561517b076bba1589e77e515343e38f682cbf37b7bdb1688fb82c9c7bfec2e current=sha256:2bf7fa5f64b4bfecf8c22badf5f7eac9a23e56841e50a9cef1ae064592ca0154
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-meeting-pack-graph-ssot-playbook --stage preview` を実行し、その後 `vibepro pr prepare . --story-id story-meeting-pack-graph-ssot-playbook --base <base-branch>` で次stageへ進む。
...
subagentの結果受領後に記録するcommand:
`vibepro review record . --id story-meeting-pack-graph-ssot-playbook --stage preview --role human_usability --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-meeting-pack-graph-ssot-playbook --stage preview --role human_usability --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-meeting-pack-graph-ssot-playbook --stage preview --role human_usability --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
