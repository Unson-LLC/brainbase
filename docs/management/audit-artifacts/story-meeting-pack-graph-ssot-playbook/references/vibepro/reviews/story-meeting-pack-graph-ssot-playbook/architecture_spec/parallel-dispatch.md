...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-meeting-pack-graph-ssot-playbook --stage architecture_spec` を実行し、その後 `vibepro pr prepare . --story-id story-meeting-pack-graph-ssot-playbook --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-meeting-pack-graph-ssot-playbook/architecture_spec/review-request-regression_risk.md`
...
`vibepro review record . --id story-meeting-pack-graph-ssot-playbook --stage architecture_spec --role regression_risk --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-meeting-pack-graph-ssot-playbook --stage architecture_spec --role regression_risk --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-meeting-pack-graph-ssot-playbook --stage architecture_spec --role regression_risk --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
