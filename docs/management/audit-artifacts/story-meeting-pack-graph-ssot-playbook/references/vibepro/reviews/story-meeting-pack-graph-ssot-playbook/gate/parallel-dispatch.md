...
- status: stale
- evidence_key: evk_31c46369341ce32199bb1650021749e2
- first_input: false
...
Stale reasons:
- risk_surface_fingerprint: risk_surface_fingerprint changed previous=sha256:2bf7fa5f64b4bfecf8c22badf5f7eac9a23e56841e50a9cef1ae064592ca0154 current=sha256:5887cc255ec6c34916368276dc6433b0ff6d6acd4451c9e454faf0662698b1e2
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-meeting-pack-graph-ssot-playbook --stage gate` を実行し、その後 `vibepro pr prepare . --story-id story-meeting-pack-graph-ssot-playbook --base <base-branch>` で次stageへ進む。
...
`vibepro review record . --id story-meeting-pack-graph-ssot-playbook --stage gate --role gate_evidence --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-meeting-pack-graph-ssot-playbook --stage gate --role gate_evidence --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-meeting-pack-graph-ssot-playbook --stage gate --role gate_evidence --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
...
Review request:
`.vibepro/reviews/story-meeting-pack-graph-ssot-playbook/gate/review-request-release_risk.md`
...
subagentの結果受領後に記録するcommand:
`vibepro review record . --id story-meeting-pack-graph-ssot-playbook --stage gate --role release_risk --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-meeting-pack-graph-ssot-playbook --stage gate --role release_risk --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-meeting-pack-graph-ssot-playbook --stage gate --role release_risk --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
