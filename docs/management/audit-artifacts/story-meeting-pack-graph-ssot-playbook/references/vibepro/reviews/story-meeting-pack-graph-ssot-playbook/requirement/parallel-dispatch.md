...
- Current head: 7215403d476d3ea40420e4d8716b53674a7ce0e4
- Dirty: false
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-meeting-pack-graph-ssot-playbook --stage requirement` を実行し、その後 `vibepro pr prepare . --story-id story-meeting-pack-graph-ssot-playbook --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-meeting-pack-graph-ssot-playbook/requirement/review-request-product_requirement.md`

Prompt:
上記review requestを読み、`requirement:product_requirement` reviewだけを実行してください。すべてのmandatory review lensを含めます。fileは編集しません。返却JSONには `status`, `summary`, `findings`, `inspection_summary`, 任意の `inspection_evidence` を含めます。

subagentの結果受領後に記録するcommand:
`vibepro review record . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role product_requirement --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role product_requirement --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role product_requirement --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
...
Review request:
`.vibepro/reviews/story-meeting-pack-graph-ssot-playbook/requirement/review-request-scope_risk.md`

Prompt:
上記review requestを読み、`requirement:scope_risk` reviewだけを実行してください。すべてのmandatory review lensを含めます。fileは編集しません。返却JSONには `status`, `summary`, `findings`, `inspection_summary`, 任意の `inspection_evidence` を含めます。

subagentの結果受領後に記録するcommand:
`vibepro review record . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role scope_risk --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role scope_risk --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role scope_risk --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
...
Review request:
`.vibepro/reviews/story-meeting-pack-graph-ssot-playbook/requirement/review-request-acceptance_e2e.md`

Prompt:
上記review requestを読み、`requirement:acceptance_e2e` reviewだけを実行してください。すべてのmandatory review lensを含めます。fileは編集しません。返却JSONには `status`, `summary`, `findings`, `inspection_summary`, 任意の `inspection_evidence` を含めます。
...
Lifecycle start command:
`vibepro review start . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role acceptance_e2e --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-meeting-pack-graph-ssot-playbook --stage requirement --role acceptance_e2e --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
