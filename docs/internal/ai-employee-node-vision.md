...
定期実行は「どちらのスケジューラで回すか」ではなく**ガバナンス要否**で分ける。

```
経路A: ノード内cron（OpenRyoko）= 体の生理現象（反射・心拍）。判断を含まず、台帳に載せない
経路B: Brainbase→Eveループ      = 業務。自律度を持ち、台帳に記録され、例外だけCompanionに浮上
```

| | 経路A: OpenRyoko cron | 経路B: Brainbase→Eveループ |
|---|---|---|
| 性質 | 体を生かす反射 | 成果を出す業務 |
| 例 | heartbeat起床、Canvas同期、ヘルスチェック、セッションGC | learn daily、SNS予約投稿、リマインド、議事録→Graph反映、mana M系 |
| 判断 | なし（決定論のみ） | あり（LLM判断・ドラフト生成を含む） |
| 自律度 | 概念不要 | 4段階のどこかに必ず属する |
| 台帳 | 載せない（ヘルスログ程度） | Run Receipt必須、Decision Events発行 |
| Companion | 一切出ない | 承認・blocked・エスカレーション時のみ浮上 |
| スケジュール定義の正本 | ノードローカル（runtime状態） | Brainbase側（agent-control-catalog）＝配布・監査可能な正本 |

**判定チェックリスト**（1つでもYESなら経路B）:

1. 外部への書き込み・送信が起こりうるか（Slack投稿・Graph書込・メール）
2. 将来 `auto_execute` へ昇格させたいと言う可能性があるか（自律度の概念が意味を持つか）
3. 失敗・滞留を佐藤がCompanionで知りたいか
4. 委任率KPIの分母に入るべき仕事か

→ 直感形:「これはAI社員の**仕事**か、**心臓の鼓動**か」。仕事をノード内cronで回すと
台帳にもKPIにも載らない**闇業務**になり、委任率の過小計測と監査抜けを生む（最も避けたい事故）。

補足:

- Companionは「経路Bを見る場所」ではなく「経路Bのうち人間の注意が必要になった項目だけが浮上する投影面」。
  正常完了の99%はRun Receiptに静かに積まれるだけ
- 既存launchdジョブ・mana M系の移設先はほぼ全部が経路B。経路Aに置いてよいのはインフラ保守系のみ
- Jinnの「cronは第一級のエージェントタスク」思想はそのまま使うと業務スケジュールの正本が
  ノードローカルに生まれ配置ポリシー（§3.4）違反になる。**OpenRyokoのcron機構は経路Bの
  dispatch受け口として使い、スケジュール定義の正本はBrainbase側に置く**（§3.2: 定義=資産、実行機構=体）
