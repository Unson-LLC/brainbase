...
related_stories:
  ...
  - story-brainbase-ontology-production-compatibility
related_docs:
...
`npm run ontology:publish -- --version <semver> --decision-id <id>`をcurrent indexとcompatibility viewを変更できる唯一のpublisher commandとする。publisherはcleanなsource commit上で実行し、releaseに記録されたproposer、decider、applier、scope、impact scopeを、Bearer tokenから既存auth middlewareが解決した`personId`へ結合する`POST /api/info/ontology/publications/authorize`へ、`source_commit_sha`、release file全bytesのSHA-256、根拠Decision IDとともに送る。Graph上の承認Decisionはpayloadに`ontology_release_version`、`ontology_release_digest`、`ontology_source_commit_sha`、`ontology_proposer_entity_id`、`ontology_decider_entity_id`、`ontology_impact_scope`、`ontology_scope_entity_id`を保持し、endpointはrequestと完全一致すること、actorとapplierの一致、同一scopeにおけるproposerのResponsible、deciderのAccountable、applierのAccountable RACIを検証する。一致しなければ409、権限を確認できなければ403とし、確認できた場合だけ`ONTOLOGY_PUBLICATION_SIGNING_PRIVATE_KEY`で署名したauthority receiptを返す。これによりcallerが送ったrelease情報をcaller自身の申告だけで承認しない。自己申告applier、service tokenだけでpersonへ結合できないprincipal、scope不一致は拒否する。

publisherはreceiptを`config/ontology/publications/<semver>.receipt.json`へ保存し、そのpathとreceipt bytesのSHA-256をindex entryへ持たせる。receiptの署名対象はschema version、Ontology version、`source_commit_sha`、release digest、Decision ID、scope entity ID、impact scope、actor/proposer/decider/applier entity ID、発行時刻であり、canonical JSON bytesをEd25519で署名する。`source_commit_sha`はrelease fileを含みpublisher生成物をまだ含まないcommitであり、receipt自身を含む最終commit SHAではない。CIの`ontology:verify`は`ONTOLOGY_PUBLICATION_SIGNING_PUBLIC_KEY`で署名と全bindingを検証するため、秘密鍵やGraph tokenを持たない変更者は正規receiptを作れない。digestは自己参照を避けるためrelease file内には置かず、index entryに`content_digest_algorithm: "sha256"`と`content_digest`として保持する。

publisherはreceipt、index、active releaseへのpointerだけを持つ`config/ontology/brainbase-ontology.v1.json`を同じpublish operationでtemp fileから生成・置換する。通常例外ではprocess内に保持したpublish前bytesから3出力を補償復元する。この保証はprocessが生存している間のoperation-level compensationであり、SIGKILL、電源断、filesystem障害を跨ぐtransactional durabilityは含まない。process termination後はclean checkout検査で再publishを停止し、operatorが`git status`で対象3 pathと`.tmp`/`.rollback`を確認して、publication commit前ならGit上のsource commitからreceipt、index、compatibility viewだけを復元する。復旧後に`npm run ontology:verify`を通すまで再publishまたはcurrent変更を禁止する。公開branch上ではsource commitの直接の子にpublisher生成物だけを変更するpublication commitを1つ作る。Ontology currentを変更するPRはmerge commit方式だけを許可し、squash mergeとrebase mergeを禁止する。PR時のCIはheadがpublication commitで`HEAD^ == source_commit_sha`かつ差分pathがreceipt、index、compatibility viewだけであることを要求する。push後または後続commit上のCIはreceiptが参照するsource commitと、その直接の子で到達可能なpublication commitを履歴から特定し、その一組の差分pathと生成内容を検証する。merge commit自体がHEADでもよいが、source/publicationの両commitが履歴に保存されていなければfailする。これによりcommit hashの自己参照を避け、通常のmerge commitを許容しつつ、squash/rebaseによる署名対象履歴の消失とrelease bytes・公開生成物間の変更混入を拒否する。`ontology:verify`はcompatibility viewがindexのcurrent version、release path、digestと一致しない場合も拒否する。CIではbase refのindex/release/receiptと比較し、receiptのないcurrent変更、receipt改ざん、既公開versionのdigest変更・削除を拒否する。PR workflowは`actions/checkout`を`fetch-depth: 0`で実行してbase commit objectを取得し、base/head SHAをverifyへ渡す。baseまたはsource commit objectを解決できなければ比較を省略せずfail closedにする。VibeProのPR mergeはこのStoryに限りmerge strategyを必須とし、検証器が保存されたcommit pairを確認してから完了とする。Graph未到達、権限不明、Decision不明はいずれもpublishをfail closedとし、既存active releaseは維持する。
...
## 2026-08-03 compatibility closure

本番snapshotで存在を確認したstorage型と、両endpoint型を確認できたlegacy relationは、1.0.0がproposedでreceipt未発行の間に`compatibility`語彙として収録する。名称だけからendpoint型を推測せず、孤児edgeは参照整合性違反として残す。既存Decisionのauthorityとscopeはpayloadへの複製を要求せず、Graphの`owned_by` person edgeと`belongs_to_project` project edgeを正本として検証する。`decided`は既存運用上の有効statusとして、`active`と同じ推論対象に含める。

このclosureはmanifestとvalidatorの適合であり、Graph factの自動修正、publication authority、署名receipt、`current`の変更を含まない。
