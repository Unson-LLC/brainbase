...
`npm run ontology:publish -- --version <semver> --decision-id <id>`をcurrent indexとcompatibility viewを変更できる唯一のpublisher commandとする。publisherはcleanなsource commit上で実行し、releaseに記録されたapplier entity IDとscopeを、Bearer tokenから既存auth middlewareが解決した`personId`へ結合する`POST /api/info/ontology/publications/authorize`へ、`source_commit_sha`、release file全bytesのSHA-256、根拠Decision IDを送る。endpointはactorとapplierの一致、対象scopeのAccountable RACI、Decisionの存在と適用対象を検証し、`ONTOLOGY_PUBLICATION_SIGNING_PRIVATE_KEY`で署名したauthority receiptを返す。自己申告applier、service tokenだけでpersonへ結合できないprincipal、scope不一致は拒否する。
...
publisherはreceipt、index、active releaseへのpointerだけを持つ`config/ontology/brainbase-ontology.v1.json`を同じpublish operationでtemp fileから生成・置換する。公開時はsource commitの直後にpublisher生成物だけを1 commitで記録し、CIはcurrent変更時に`HEAD^ == source_commit_sha`かつ`source_commit_sha..HEAD`の変更pathがreceipt、index、compatibility viewだけであることを要求する。これによりcommit hashの自己参照を避けつつ、release bytesと公開生成物の間への変更混入を拒否する。`ontology:verify`はcompatibility viewがindexのcurrent version、release path、digestと一致しない場合も拒否する。CIではbase refのindex/release/receiptと比較し、receiptのないcurrent変更、receipt改ざん、既公開versionのdigest変更・削除を拒否する。PR workflowは`actions/checkout`を`fetch-depth: 0`で実行してbase commit objectを取得し、base/head SHAをverifyへ渡す。baseまたはsource commit objectを解決できなければ比較を省略せずfail closedにする。Graph未到達、権限不明、Decision不明はいずれもpublishをfail closedとし、既存active releaseは維持する。
