...
1. requestとDAGをJSON-compatible snapshotへ複製する。関数を含むrunner登録はsnapshot対象外とする。
2. `validateJudgmentDAG`で構造と順序を確定する。
...
6. caller指定`run_id`、DAG ID/version、DAG/input snapshot、execution order、runner version、node contract refs、outputを一つのdeep-frozen run recordとして返す。
...
- runner登録欠落、不正request、不正JSON境界、runner失敗は`JudgmentDAGExecutionError`のmachine-readable codeで区別する。
- preflight failureではrunner call countが0でなければならない。
- runner failure後の再試行、partial record永続化、補償処理はこのStoryでは扱わず、元errorをcauseとして保持してfail loudする。
