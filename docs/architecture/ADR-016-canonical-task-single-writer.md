...
- 初回migration前に`docs/runbooks/canonical-task-cutover.md`と`scripts/preflight-canonical-task-cutover.js`で旧Brainbase、Mana、MCP、運用scriptを停止・排水する。before-enableは全必須回帰をcurrent HEAD証跡へ束ね、明示enableがatomicに成功するまで全mutationを503にする。rollbackは最初に明示disableし、旧直接writerを復活させない。
...
この追加gateにより、writer tokenを持つprocessでもmigration・回帰・store identityの確認前には書けない。
証跡欠落や再起動時の不一致は可用性よりfail-closedを優先し、readだけを維持する。
