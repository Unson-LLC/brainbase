...
release bytesとpublication実装はGit、人物・Decision・RACIはGraph SSOT、署名秘密鍵はInfisical production、検証公開鍵はGit管理の`config/ontology/trusted-public-keys.json`、実効状態は署名receiptと`config/ontology/index.json`を正本とする。これらを別の正本へ複製しない。
...
4. production signing private keyをInfisicalからruntimeへ投影し、認証済みpublication endpointがEd25519 receiptを発行する。対応する非機密のpublic keyはreceiptの`key_id`に結合したGit管理の信頼ストアから全runtimeへ配布する。
...
release digest不一致、source commit不一致、認証actor不一致、RACI不足、Decision binding不足、receiptの`key_id`に対応する公開鍵不存在、署名検証失敗、Graph violation、partial snapshotではreceiptを作らずcurrentを変更しない。
