...
- runtime 2.4で必要なknowledge/stateが揃っていても、最終assistant回答にHost監査行が欠けた初回Stopは正確な監査ブロックと本文保持指示を返し、一度だけ再回答を求める。Stopは修復後の実回答を検証できるまでcomplete final receiptを確定しない。状態や実行契約の不一致は監査表示だけの修復対象にしない。
