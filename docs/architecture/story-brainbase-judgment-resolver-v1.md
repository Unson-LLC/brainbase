...
- A missing required route or a final answer that omits, duplicates, or reorders a stored owner-visible audit line returns `decision:block`, including on an active repeated Stop, until the repairable display contract is satisfied; no incomplete final receipt is written. An orphan Stop still fails visibly instead of silently returning `{}`. Transaction-acquisition timeout follows that terminal fail-closed boundary.
