...
- **server enforcement**: APIはhost署名、認証、project scope、personal owner visibility、input reconciliation、manifest integrityを必ず強制する。receiptはaction authorizationではなく、既存toolのauth/approvalを置換しない。
