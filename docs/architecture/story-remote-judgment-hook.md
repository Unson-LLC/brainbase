...
- `PostToolUse`と`PostToolUseFailure`はcanonical出力の非空`systemMessage`を監査記録receiptとして要求し、未記録なら503にする。利用者向け監査行を生成しない内部状態記録の完全名`mcp__brainbase__brainbase_judgment_state_record`だけは空出力を受理し、短縮名や他toolへ例外を拡張しない。
