...
## 実DBからVibePro受入までの連続試験

### 今回の最小Spec

前節で別々に確認した保存・発行と受入を、一つの試験入力でつなぐ。利用者が意図した成果を、保存や引渡しの途中で取り違えないことを確認する。製品の機能・権限・本番設定は変更しない。

1. 専用の一時PostgreSQLで、RLSを迂回できないアプリケーションロールから元の判断記録とOutcomeCaseを保存する。
2. 認証付き採用APIと発行APIを順に呼び、その発行応答を再構成せずVibeProの実受取へ渡す。
3. VibeProの保存結果を読み戻し、成果案件の7項目、出典、署名検証結果を確認する。技術完了の項目を勝手に追加せず、元のOutcomeCaseは変更されない。PR準備が生成する技術完了falseは、この連続試験では未検証とする。
4. 発行応答を改ざんした場合は受入を拒否し、受取側に契約の保存結果を残さない。
5. 接続するVibeProのcheckoutは明示指定する。未指定によるskipは接続成功に数えず、不正な指定は失敗として扱う。

トークン検証器、採用許可の初期登録、鍵は試験専用とする。本番認証・本番権限の検証、実際の議事録・Mana返信、両リポジトリを取得するCI配線は対象外である。

### 2026-09-04の実行結果

- 実PostgreSQLから認証付き採用・発行APIを通し、HTTP応答本文を変更せずVibeProのinboxへ保存した。実consumerが署名検証して保存し、context、設定、受取記録、消費台帳を読み戻した。7項目と出典・digestの一致に加え、実検査関数の `trusted` を確認した。
- 改ざん入力は未採用の受取側で拒否され、設定は不変、契約の保存ファイルは存在しない。元のOutcomeCaseも連続試験前後で不変だった。
- 親の再実行で実DB試験9件成功、skip 0（従来8件＋連続試験1件）。VibeProは `733764ffda329cb01c71c999691476022e9b2666` の変更していないcheckoutを使用した。
- 最初はVitestの外部ESM読込みで失敗した。通常のNodeで同じmoduleが読めることを確認し、試験専用のCJS helperからNodeの読込みを使って解消した。製品コードと署名検証は変更していない。
- ソースcheckoutからの `preparePullRequest` は実行元の完全性検査で `runtime_mismatch` となる。この拒否を確認し、検査を解除していない。この連続試験は受取・保存・信頼性検査までであり、PR判定・技術完了・本番成果の合格ではない。
- 別のTerraによる独立レビューで、受入と技術完了falseの証拠範囲を混同しないよう指摘を受け、この節を修正した。
- 接続先未指定では既存8件成功・連続試験1件skip、不正なmodule指定では連続試験が失敗（終了値1）となることを確認した。skipや接続失敗を受入成功へ集約しない。

再実行は依存関係を導入済みの両checkoutと専用DBを用い、Brainbase側で次を実行する。鍵はテスト内の専用値で、本番の鍵や許可表は使用しない。

```sh
RUN_OUTCOME_CASE_DB_TESTS=1 \
OUTCOME_CASE_DATABASE_URL='<専用の一時DB接続先>' \
