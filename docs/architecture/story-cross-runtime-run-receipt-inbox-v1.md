...
9. Repository startup inserts missing seed workflows only inside a dedicated pre-publication initialization transaction that uses the same file-wide lease, owner context, reload, single commit, and `finally` release rules. It never overwrites existing ledger content. Later default-automation repair uses the normal reentrant transaction. If a receipt workflow is absent, create/project-check it, then create run and audit in the ingest repository transaction.
