...
4. Acquire a repository receipt identity lock using `workspace_id=run_receipt:<run.project_id>`, `workflow_id=<deterministic run id>`, and a unique ingest owner. The `run_receipt:` namespace prevents collision with other workflow-lock users while preserving project scope. Acquisition retries for a bounded interval. JsonFile stale-lock recovery serializes lock-file mutation and atomically quarantines the expired file; if another repository wins the now-empty path, recovery returns contention and never deletes that fresh lock. Release uses the same mutation boundary and owner check. While that lock is held, enter the repository-wide write transaction before duplicate lookup, conflict comparison, workflow/run/audit persistence, or returning the committed duplicate. Missing lock/transaction capability fails without writes. Identity-lock timeout returns retryable HTTP `503` with `Retry-After`; contract/conflict failures remain `400`. Either timeout path leaves the ledger unchanged.
