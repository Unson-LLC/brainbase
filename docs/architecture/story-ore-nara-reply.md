...
状態確定前のHost判定 ---- 権限・秘密・不可逆操作 --------------------> 本人へ確認
       ...
       v
Personal KG検索 ---- 該当なし・取得失敗・新しい価値判断 ------------> 本人へ一度だけ確認
       |
       | 既存の回答あり
       v
...
- Stopまで待たず、`brainbase_judgment_state_record`が確認待ちを確定する時点でもPersonal KGの事前検索を検証する。これにより、質問を一度表示してからStopが差し戻す二重表示を防ぐ。
...
8. `required_input_unavailable`の状態記録はPersonal KG検索前には拒否され、回答取得後も質問ではなく実行継続へ戻される。
