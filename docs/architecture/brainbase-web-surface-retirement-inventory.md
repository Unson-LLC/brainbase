...
| `public/device.html` | device code検証、Google Workspace OAuth、consent、approve/denyだけを持つ。approveはBearer tokenを検証し、承認者identityをサーバー側で確定 | 最小Web | ブラウザ本人確認/OAuthのためWebが正規 | `keep_minimal_web` | `TSK-WEBRET-008`完了。5状態以外のUIなし、日常一覧・設定導線なし、caller提供identityを不採用 |
