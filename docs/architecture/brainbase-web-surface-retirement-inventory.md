...
`TSK-WEBRET-001`から`007A`、`009`、`010`まで完了した。Workflow Web、旧session shell、SNS専用Web UI、Admin Web、Setup Webは削除済みであり、Meeting Automation、Run台帳、SNS Core API/ledger、admin/setup REST backend、config正本はCoreとして残る。次は`TSK-WEBRET-008`として`device`が本人確認、OAuth、pairing以外の能力を持たないことを回帰検査する。
