...
- `mcp/brainbase/src/server.ts`のBrainbase MCPはGraph、Personal KG検索に加え、認証済みproject catalog、Run Receipt Inbox/history/diagnosis、Automation Run detail/human-step resolve、Meeting Automation diagnosis、bootstrap config、admin readを提供する。Wiki検索は退役済みで、旧tool名の拒否処理を現役の検索経路として扱わない。
- Admin visualizationとsetupのREST APIはMCP backendとして維持する。device authorizationはブラウザ本人確認/OAuthのため最小Webを正規面とする。SNSは後続の[SNS廃止仕様](../specs/retire-sns-spec.md)に従いCore APIも全操作410とする。旧台帳は保全し、共通サービスまで一括削除しない。
...
| `public/device.html` | device code検証、設定済みproviderのOAuth、consent、approve/denyだけを持つ。表示は「組織アカウントでログイン」。approveはBearer tokenを検証し、承認者identityをサーバー側で確定 | 最小Web | ブラウザ本人確認/OAuthのためWebが正規 | `keep_minimal_web` | `TSK-WEBRET-008`完了。5状態以外のUIなし、日常一覧・設定導線なし、caller提供identityを不採用 |
| `public/sns-growth.html` | 廃止済み。review pack、post一覧/更新、publish、feedback、account default/healthを持つ専用cockpitだった | 後継SNS製品面なし。旧台帳は保全 | Web UIに続きCore APIも退役し、全操作410。共通サービスは維持 | `deleted_ui_retired_api` | `TSK-WEBRET-010`でUI削除。後続整理で旧routeと専用テストを削除し、static 404とproduction API 410・副作用なしの契約テストを維持 |
...
7. `sns growth`: 専用MCP/Web製品面を追加しない。後続のSNS廃止仕様によりAPIは全操作410、専用CLIは副作用前に退役応答。保存台帳は保全し、稼働ジョブの停止・本番readbackは別途確認する。
...
| `TSK-WEBRET-010` | SNS Growth専用Web UIを廃止 | 当時のUI退役は完了。page、view、旧shell接続、共有旧CSS、UI/E2E testsを削除しstatic 404を固定。当時維持したCore APIは後続のSNS廃止仕様で410へ退役 |

`TSK-WEBRET-001`から`010`まで完了した。Workflow Web、旧session shell、SNS専用Web UI、Admin Web、Setup Webは削除済みであり、Meeting Automation、Run台帳、admin/setup REST backend、config正本はCoreとして残る。SNS Core APIは後続仕様で退役し、保存台帳は保全する。これは本番配備・ジョブ停止の完了を意味しない。唯一のproduction Web UIである`device`は本人確認、OAuth、consent、pairingだけに限定し、承認者identityは認証済みtokenからサーバー側で確定する。

旧session/transcript UI island、`public/dist` bundle、起動時build hook、専用E2Eも削除した。さらに[残存整理](../verification/legacy-surface-completion.md)で入口を失った121資材を削除した。回帰テストは`public/`全体をdevice HTML/controller、favicon、型検査設定の4ファイルだけに制限する。静的配信は型検査設定を除く3資材の許可リストとし、配備先に旧資材が残っても再公開しない。
