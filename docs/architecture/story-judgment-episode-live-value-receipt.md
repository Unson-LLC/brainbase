...
6. 実際の中断差し戻しとvalue proofが揃った場合だけ判断レシートを描画する。差し戻し済みruntime 2.4 continuationでvalue proofが欠けた`completed` state PostToolUseは、`decision:block`と修復理由をそのままHook出力へ返し、finalを作らない。value proofと新しい最後のstateが揃った時点で、未来のStop有無に依存せず確定し、`post_tool_use_system_message`として表示する。後続Stopは同じimmutable finalをreplayする。
...
- state PostToolUseでの自動確定は、既に差し戻した不要確認または未完了作業があり、状態が`completed`のときだけ許す。これはStop欠落時だけの推測的fallbackではなくruntime 2.4 continuationの正規経路である。`waiting_human`や通常turnをStopなしで確定しない。
- 必須value proofが欠けたstate PostToolUseの`decision:block`を`systemMessage`だけへ変換しない。Codex Hostがツール後ブロックとして扱い、モデルがvalue proofと最後のstateを補完できる契約を保つ。
...
- Integration: 実Hook wrapper経由で不要確認のStop差し戻し、実行証拠、最後の`completed` state PostToolUse、`post_tool_use_system_message`、`answer_digest=null`までを同じepisodeで完了し、二度目のStopを必要としない。
- Integration: value proofより先に完了stateを送ると実Hook wrapperが`decision:block`を返し、proofと再state後に同じepisodeを完全確定する。
- Unit: Desktop履歴のfileChangeと単一readを厳密な3識別子で結合し、最後のcompleted state PostToolUseから成果確認済みレシートを確定できる。
