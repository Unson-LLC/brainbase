...
6. 実際の中断差し戻しとvalue proofが揃った場合だけ判断レシートを描画する。差し戻し済みruntime 2.4 continuationは、未来のStop有無に依存せず、最後の正常な`completed` state PostToolUseで確定し、`post_tool_use_system_message`として表示する。後続Stopは同じimmutable finalをreplayする。
