...
6. 実際の中断差し戻しとvalue proofが揃った場合だけ判断レシートを組み立てる。差し戻し済みruntime 2.4 continuationでvalue proofが欠けた`completed` state PostToolUseは、`decision:block`と修復理由を返してfinalを作らない。`PostToolUse`はcompleted stateを記録するだけで、value proofと新しい最後のstateが揃った後も、後続`Stop`が最終assistant回答の監査ブロックを検証して確定する。
...
- Integration: value proofより先に完了stateを送ると実Hook wrapperが`decision:block`を返し、proofと再state後もfinalを作らず、後続Stopで同じepisodeを完全確定する。
