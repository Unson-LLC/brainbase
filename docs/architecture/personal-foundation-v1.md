...
> 2026-09-25改訂：「共有・昇格・削除のAPIは今回追加しない」のうち共有・昇格は、横断ADR（brainbase-project ADR-003 D4）で改めた。v1を個人の記憶の正本とし、v1に共有・昇格の受け口を加える（Planned、未実装）。削除のAPIは対象外のまま。詳細は「ローカルと組織」を参照。
...
共有・昇格・削除のAPIは初回実装に含めない。組織管理者に本人本文への権限を追加しない。

共有・昇格は、横断ADR（brainbase-project ADR-003 D4）により、v1に受け口を加える計画とした（Planned）。現在のv1に共有・昇格のAPIはない。
- 個人の記憶の正本はv1とする。既存の保存先の記憶は自動で移行しない。
- 昇格は、本人の同意と組織のレビューを分けた二段階とする。共通の受け口と処理はOSSが持ち、組織のレビューは組織版が持つ。
- 受け口を加えても、組織管理者に本人本文への権限は与えない。
- 削除のAPIは対象外のまま。
