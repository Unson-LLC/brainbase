...
Add `onboard:start` as a composition layer over existing onboarding primitives. The command produces a Japanese first-run plan and copyable next commands; it does not introduce a new runtime, provider connector, schema, hosted backend, or UI.

## Boundaries

- `src/cli.ts` owns argument parsing, Personal OS initialization, local command availability checks, and dispatch.
- `src/guided-onboarding.ts` owns the guided output model, Japanese rendering, source readiness summaries, and next-command assembly.
...
3. The CLI diagnoses local source readiness without importing provider data.
4. The guided renderer emits Japanese interview prompts, source readiness, approval gates, and next commands.
5. The user or agent runs the suggested commands after review.
