...
監査側の故障線は、実行権限の線を切断しない。required capability不足や未完了作業のStop制御は維持する。

回答本文とHost監査は別の表示チャネルに分ける。モデルは本文を1回だけ生成し、Stopは最新の監査を`systemMessage`で投影する。監査行が回答本文にない、または一致しないことだけで本文を全文再送させない。

```text
モデル --------------------> 回答本文（1回）
Host Stop --> 監査の確定 --> systemMessage（本文なし）
```
