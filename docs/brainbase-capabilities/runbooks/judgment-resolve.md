...
6. `Stop` validates the event set and the actual `last_assistant_message`, then atomically creates one complete final episode receipt only when the episode-start contract is satisfied. The answer must begin with the stored `🧠` line followed by every stored `📚`/`⚠️` line in journal-commit order, with no extra copies. If required `knowledge.resolve` or that rendered audit prefix is missing, every repairable Stop returns `decision:block`, including when `stop_hook_active=true`, and writes no final receipt. When knowledge is optional and zero Brainbase calls were recorded, the episode-bound prefix includes `📚 Brainbase未参照: 必須参照なし・実呼び出し0回 ✓`. For an audit-only repair, the Host stores the normalized non-audit body digest and character count—not the answer text—and refuses completion if the regenerated answer deleted, summarized, or replaced that body. Orphan Stop and transaction-acquisition timeout remain terminal fail-closed errors instead of returning `{}`.
...
The additional context and `PostToolUse.systemMessage` guide the model, but they are not accepted as owner-visible evidence by themselves. `Stop` checks the final answer and keeps requesting a corrected rendering when the stored lines are missing, duplicated, or out of journal-commit order. For an audit-only retry, it also requires the first rejected answer's non-audit body to remain unchanged after presentation normalization. A short acknowledgement cannot replace the detailed implementation report. Trailing spaces or tabs at line ends are presentation-equivalent; the audit text, order, and multiplicity remain exact. An active repeated Stop returns `decision:block` for repairable display errors instead of a non-zero Hook failure that Codex would treat as non-blocking.
...
- Missing required knowledge or an invalid owner-visible audit prefix returns `decision:block` on every repairable Stop, including the active repeated Stop, without a final receipt.
- An audit-only repair that drops or summarizes the original answer body also returns `decision:block`; the model must prepend the required audit lines and preserve the original body.
