...
The completed Stop Hook response contains one Host-rendered `systemMessage` with the stored initial judgment line and every journaled `📚`/`⚠️` line in commit order, followed by the optional judgment value-proof and attention surfaces. This `systemMessage` is the canonical owner-visible audit/value surface exposed through the Codex Hook UI or event stream. The model-authored answer body does not start with or duplicate these lines. Intermediate commentary does not carry the completed audit block:
