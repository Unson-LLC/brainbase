...
7. The owner audit contains a redacted excerpt of at most 26 Unicode code points and the judgment result. Codex is required to place its exact `🧠 Brainbase参照:` display line first in every user-facing response.
...
```text
🧠 Brainbase参照: 直前の「ログイン後の白画面」を参照 → 実装依頼として継続 ✓
```
