...
The additional context and Hook `systemMessage` guide the model and may show short in-progress status. `Stop` checks and binds the exact Stop Hook-visible business answer before the Host adds its journal-rendered audit namespace as a separate `systemMessage`; the model neither reproduces nor verifies those lines. Transcript comparison may exclude only one complete trailing `<oai-mem-citation>...</oai-mem-citation>` block added later by the Codex application; an incomplete, embedded, or multiple citation block remains part of comparison and fails closed on mismatch. Every repairable rejection records one Host-owned `stop_repair` marker, and a model-authored `🛠️` line without that marker is rejected. A journal-proven successful retry appends exactly one `🛠️ Stop修復: 最終回答を1回差し戻し → 修復完了 ✓` to the Host-rendered audit block; a normal turn has no repair line. A short acknowledgement cannot replace the detailed implementation report. Stop blocks at most once: an active repeated Stop whose substantive contract remains incomplete finalizes as `audit_degraded` (with an appended `⚠️ 監査縮退: <reason>` line) and exits 0.
...
- Missing required knowledge or another substantive capability returns `decision:block` on the first repairable Stop without a final receipt; an incomplete active repeated Stop (continuation marker already exists) finalizes as `audit_degraded` and exits 0. `audit_degraded` finals never reach the knowledge outbox.
- A repair that drops or summarizes the original business answer is rejected on the first Stop; an incomplete active retry converges to `audit_degraded` rather than reopening the repair loop.
