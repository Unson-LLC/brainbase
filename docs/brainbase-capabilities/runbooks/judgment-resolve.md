...
6. `Stop` first checks for an existing episode. If none exists, it may recover only one complete current-turn `<codex_delegation>` from a trusted `codex_app` `create_thread` or `send_message_to_thread` output. The transcript must contain only one connected session-alias component; a foreign component, multiple candidates, malformed envelope, other tool, or turn mismatch remains a true orphan. A recovered episode records `episode_origin=stop_delegation_recovery` and `route_application=post_generation_recovery`, and the final receipt preserves both fields. This route governs the Stop decision and continuation only; it never claims to have guided the already-produced pre-Stop answer. `Stop` then validates the event set, the actual model-authored `last_assistant_message`, and the required capability, autonomy, continuation, and business-body evidence before atomically creating one complete final episode receipt. The Host does not require the model-authored answer to repeat or prefix the audit lines. On a successful Stop it returns one `systemMessage` containing the complete journal-derived owner audit block, followed by the optional judgment value-proof surface and companion attention surface; this is the owner-visible surface exposed through the Codex Hook UI or event stream. `final.json` records `owner_audit_source=stop_hook_system_message`; `answer_digest` hashes the exact raw model-authored `last_assistant_message`, while normalized business-body checks remain separate and never replace that digest. If a `continue` receipt detects an unnecessary user question, the first Stop returns `decision:block` plus `systemMessage: 🔁 確認不要と判定しました。回答を差し戻して処理を続けています` and stores count `1`, trigger, Resolver reason, and `requested` status in immutable `continuation.json`. The retry must perform the remaining safe work and record a journaled continuation completion marker; the Host then renders `🔁 自律継続: 不要な確認を1回差し戻し → 継続完了 ✓` after the Brainbase audit lines in its `systemMessage`, and final records the same structured evidence with `completed` status. Every repairable Stop rejection also stores `stop_repair: { count: 1, status: requested }`. A successful retry records the journaled `🛠️ Stop修復: 最終回答を1回差し戻し → 修復完了 ✓` marker with `completed` status; the Host renders it in the same `systemMessage`. A `🔁` or `🛠️` line authored by the model without matching journal evidence is rejected. If required `knowledge.resolve`, autonomy, continuation, or business-body evidence is missing, the first repairable Stop returns `decision:block` and writes no final receipt. Stop blocks at most once per episode: if the `stop_hook_active=true` retry finds a continuation marker already exists and is still incomplete, it no longer regenerates forever or exits non-zero with `judgment_stop_repair_exhausted` — it converges to a finalized `audit_degraded` receipt (`degradation_reason` names the first missing capability) with an appended `⚠️ 監査縮退: <reason>` audit line in the Host `systemMessage`, and exits 0. When knowledge is optional and zero Brainbase calls were recorded, the Host-rendered episode-bound audit block includes `📚 Brainbase未参照: 必須参照なし・実呼び出し0回 ✓`. For an audit-only repair, the Host stores the normalized business-body digest and character count—not the answer text—after removing only the leading Host audit namespace block, including malformed variants. It refuses completion if the regenerated answer deleted, summarized, or replaced that body. A true orphan Stop cannot fabricate the model-generation-before route: it requests the exact degraded warning at most once, then records an immutable `audit_degraded` receipt and exits successfully so a long-running task does not require a new task. The warning explicitly says that work continues and that creating a new task or operating Hooks is unnecessary. `audit_degraded` is never a complete final, retrieval success, task completion, prior finalized judgment, or action authorization. Identity, diagnostic-integrity, episode-integrity, and transaction-acquisition failures remain terminal fail-closed errors.
...
- `final.json` binds the immutable event-set digest, the exact raw model-authored `last_assistant_message` digest, the owner-display status/source, and records `complete`. The Host-rendered `systemMessage` is a separate owner-visible surface and is not included in `answer_digest`. Historical incomplete journals remain readable but are not newly created.
...
The completing lifecycle event returns one Host-rendered `systemMessage` with the stored initial judgment line and every journaled `📚`/`⚠️` line in commit order, followed by the optional judgment value-proof and attention surfaces. A normal path uses the completed Stop Hook response; a previously rejected runtime 2.4 continuation uses its last valid completed state `PostToolUse`. This `systemMessage` is the canonical owner-visible audit/value surface exposed through the Codex Hook UI or event stream. The model-authored answer body does not start with or duplicate these lines. Intermediate commentary does not carry the completed audit block:
...
The additional context and Hook `systemMessage` may show short in-progress status. The final owner-visible audit/value evidence comes from the lifecycle event recorded in `owner_audit_source`: a normal completed Stop, or the last completed state PostToolUse for a previously rejected runtime 2.4 continuation. `Stop` checks and binds the exact Stop Hook-visible model answer for required capabilities, autonomy, continuation, and business-body safety before the Host adds its journal-rendered audit namespace; it does not request a regenerated answer merely because the Host-rendered audit lines are absent from that body. Stop finalization records `owner_audit_source=stop_hook_system_message` and binds the exact raw `last_assistant_message` in `answer_digest`. Continuation PostToolUse finalization records `owner_audit_source=post_tool_use_system_message` and preserves the unavailable answer digest as `null`. Transcript comparison may exclude only one complete trailing `<oai-mem-citation>...</oai-mem-citation>` block added later by the Codex application; an incomplete, embedded, or multiple citation block remains part of comparison and fails closed on mismatch. For an audit-only retry, the Host also requires the first rejected answer's business body to remain unchanged after presentation normalization. A leading reserved line beginning with `🧠 判断参照:`, `📚 Brainbase`, `⚠️ Brainbase`, `🔁 `, or `🛠️ ` is presentation metadata even when malformed; the same text after the business body starts remains body content except that an unjournaled `🔁` or `🛠️` completion claim is always rejected. Every repairable rejection records one Host-owned `stop_repair` marker. Journal-proven successful retry markers are rendered by the Host in `systemMessage`; a model-authored `🛠️` line without that marker is rejected and a normal turn has no repair line. A short acknowledgement cannot replace the detailed implementation report. Trailing spaces or tabs at line ends are presentation-equivalent; the Host keeps the audit text, order, and multiplicity exact. Stop blocks at most once: an active repeated Stop whose repair is still incomplete no longer exits non-zero with `judgment_stop_repair_exhausted` — it finalizes as `audit_degraded` (with an appended `⚠️ 監査縮退: <reason>` line in `systemMessage`) and exits 0.

差し戻し済みruntime 2.4 continuationでは、必須value proofが揃った後の最後の正常なstate PostToolUseが正規の確定境界となる。value proofが欠けたstate PostToolUseは`decision:block`をそのまま返してfinalを作らず、モデルはvalue proofと新しい最後のstateを記録する。Hostは将来Stopが届くかを推測しない。完全な経路は`owner_audit_source=post_tool_use_system_message`、`answer_digest=null`を記録し、完全な監査・価値表示をPostToolUseの`systemMessage`で返す。後続Stopは同じimmutable finalをreplayする。差し戻しのない通常経路をStopで確定した場合は`owner_audit_source=stop_hook_system_message`と正確な回答digestを記録する。
...
- Orphan `PostToolUse` events are not attached to an episode; each leaves a digest-only orphan marker and visible warning without consuming the Stop repair state. A late `UserPromptSubmit` after that marker is blocked with `judgment_orphan_tool_event_start_conflict`: the marker intentionally lacks the raw event required to reconstruct a complete audit, so starting a normal episode would falsely claim completeness. An episode-less Stop first attempts the bounded Codex App delegation recovery above. If it cannot prove one current-session/current-turn candidate, the true orphan `Stop` writes a digest-only diagnostic and returns one visible `systemMessage` block while preserving the answer body. The next Stop for that identity records `audit_degraded` and exits 0 even if the warning/body verification booleans are false; it never fabricates `.final.json` or asks the operator to create a new task. Replayed first-phase payloads do not create another repair loop. Missing identity, diagnostic tampering, conflicting immutable evidence, and other Stop integrity failures remain explicit non-zero failures and may still suggest checking Hook trust in Settings → Hooks.
- Missing required knowledge, autonomy, continuation, or business-body evidence returns `decision:block` on the first repairable Stop without a final receipt; an incomplete active repeated Stop (continuation marker already exists) no longer exits non-zero with `judgment_stop_repair_exhausted` — it finalizes as `audit_degraded` and exits 0. A completed Stop's owner-visible audit/value surface is the Host's `systemMessage`, and `audit_degraded` finals never reach the knowledge outbox.
...
```json
{
  ...
  "system_message_digest": "sha256:<sha256 of the exact system_message>",
  "occurrences": 1
}
```
