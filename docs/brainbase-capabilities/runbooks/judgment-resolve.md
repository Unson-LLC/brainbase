...
The additional context and `PostToolUse.systemMessage` guide the model, but they are not accepted as owner-visible evidence by themselves. `Stop` checks the final answer and keeps requesting a corrected rendering when the stored lines are missing, duplicated, or out of journal-commit order. Trailing spaces or tabs at line ends are presentation-equivalent; the audit text, order, and multiplicity remain exact. An active repeated Stop returns `decision:block` for repairable display errors instead of a non-zero Hook failure that Codex would treat as non-blocking.
