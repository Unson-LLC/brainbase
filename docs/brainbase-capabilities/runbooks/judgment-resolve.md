...
- Missing required knowledge, autonomy, continuation, or business-body evidence returns `decision:block` on the first repairable Stop without a final receipt; an incomplete active repeated Stop (continuation marker already exists) no longer exits non-zero with `judgment_stop_repair_exhausted` — it finalizes as `audit_degraded` and exits 0. A completed Stop's owner-visible audit/value surface is the exact assistant answer, and `audit_degraded` finals never reach the knowledge outbox.
