...
| `judgment.resolve` | Codex Host opens one canonical-context-bound judgment episode before model generation; the internal-LLM-free Resolver deterministically selects the initial route, and `PostToolUse` records completed tool calls as execution evidence without finalizing. `Stop` verifies that the exact final assistant answer begins with the journal-derived owner audit block and is the sole finalization boundary. The resulting receipt is non-authorizing. Claude Code remains a future Host-adapter candidate. |
