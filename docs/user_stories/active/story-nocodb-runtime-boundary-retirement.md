...
Canonical Tasks keep their existing explicit backend/cutover contract; this
cleanup does not silently switch an unconfigured environment to PostgreSQL.

## Acceptance criteria

- Canonical backend selection is unchanged: an unset backend remains NocoDB,
  and PostgreSQL requires explicit selection and the existing cutover evidence.
  Removing this compatibility default is a separate migration change.
...
backend-selection preservation, the shared cursor helper, and the legacy Brainbase action
