...
- **INV-6**: repairableなowner監査不足は、Codexがblockとして解釈する`decision:block`で継続する。
...
- **C-5**: active再Stopでrequired knowledgeまたはowner auditが不足する場合も`decision:block`を返し、final receiptを作らない。
...
- **S-5**: first Stopとactive再Stopの修復可能な不足は`decision:block`で継続し、complete finalなしに`task_complete`へ進ませない。
