...
- **C-2**: Hostが列挙する必須3イベントが各1件、same command、enabled、expected matcher、`trusted|managed`ならexit 0と`ready_for_fresh_task`を返す。`PostToolUseFailure`はHostが列挙する場合だけ同じ検証を必須とし、未列挙なら互換性差分として報告する。
- **C-3**: trust不成立ならexit nonzero、`trust_required`、現在Hostが列挙する必須Resolver Hookの承認案内を返す。
...
- **S-1**: fake Codex app-serverが必須3件を`trusted`で返すとreadinessは`ready_for_fresh_task`になり、`PostToolUseFailure`未列挙を互換性差分として返す。4件を返すHostでは4件すべてを検証する。
