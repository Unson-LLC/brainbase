...
- `POST /api/info/ontology/publications/authorize` authority receipt発行。requestは`release_version`、`source_commit_sha`、`release_digest`、`decision_id`、`scope_entity_id`、`impact_scope`、`proposer_entity_id`、`decider_entity_id`、`applier_entity_id`を必須とし、actorはrequest bodyではなく既存auth contextの`personId`から取得する。Decisionはversion/digest/source commitに加えてproposer、decider、impact scope、適用scopeを完全にbindする。Graph RACIはproposerのResponsible、deciderのAccountable、applierのAccountableを同一scopeで検証する。responseの署名payloadにもこれら全bindingとactorを含め、`signature_algorithm: ed25519`、署名、key IDを持つcanonical receiptとする。
...
専用pathはv1でkernelを直接呼ばないが、manifest contract testが上表の出力語彙を拘束する。current不在時にGraphへ書く既存runtime pathは、成功responseの既存fieldを維持したまま`guard_status: inactive_no_current`を必須追加し、route/controller contract testはこのfieldがないpre-fix responseを失敗させる。inventory verifierは上記探索範囲の全検出fileとmatrixの双方向一致を検証し、Companion writerを含む未検出のallowlist entryと未分類writerのどちらもfailする。`deferred` writerも検査を免除せず、canonicalな`vocabulary.types` / `vocabulary.relations`と、非canonicalリテラルの`classified_literals.compatibility` / `internal` / `rejected`をすべて宣言する。未分類・重複分類・検出されなくなったstale分類はfailし、分類とdeferred理由をinventory reportへ出力する。silent bypassではなく、未登録値はtest/verify gateのfindingにする。
...
- Graph authority endpointはactor/applier一致、同一scopeにおけるproposerのResponsible、deciderのAccountable、applierのAccountable RACI、Decisionを検証し、Decision payloadの`ontology_release_version`、`ontology_release_digest`、`ontology_source_commit_sha`、`ontology_proposer_entity_id`、`ontology_decider_entity_id`、`ontology_impact_scope`、`ontology_scope_entity_id`とrequestが完全一致する場合だけ、それらをcanonical JSONへbindしたEd25519署名receiptを返す。不一致は409、権限不足は403とする。`source_commit_sha`はreleaseを含みpublisher生成物をまだ含まないclean commitであり、receipt自身を含むcommit SHAではない。private keyはserver secret、public keyはCI verify環境で与える。
...
11. publication lifecycle integration: currentなしから開始し、明示versionでproposedを読めること、actor/applier一致、proposer/decider/applier RACI、Decisionの全publication bindingの承認、唯一のpublisherによるreceipt/index/compatibility view生成、currentのactive解決、active-current generic split-write guardまでを1つのfixtureで通す。途中の実効状態`proposed`→`approved`→`active`も観測する。
