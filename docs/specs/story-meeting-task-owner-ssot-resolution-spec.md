...
- project scoped lookupでは `tech-knight` / `techknight` / `tech_knight` のようなproject code variantを検索条件とGraph access scopeの両方に入れて試し、さらにglobal People SSOT検索結果を常にmergeする。
...
- FM-008: project codeが `tech-knight` と `techknight` で揺れる場合はvariant lookup、variant access scope、global mergeによりalias候補を失わない。
