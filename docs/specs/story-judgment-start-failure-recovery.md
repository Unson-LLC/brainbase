...
Node不在またはHostのimport失敗はHost内の診断保存より前に起きるため、入口で安全な固定診断を返す。
通常監査では非zero・`continue:false`を維持し、説明継続の成功と扱わない。
記録専用では空のHook応答・安全なstderr・exit 0とし、記録失敗で会話を停止しない。
importの事前確認と本実行の間の環境変化、および本実行だけで起こる失敗は事前確認では保証しない。
