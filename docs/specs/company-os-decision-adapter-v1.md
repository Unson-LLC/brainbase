...
は conflict とし、更新や supersede は別契約に委譲する。`event_id` は互換 response の
識別子で、canonical event store を新設しない。
...
`readDecision` は canonical Decision と Graph entity の readback を確認する。sidecar が
...
のschema不正、trusted validator の拒否、Graph v1、既存ID、readback不一致は全て失敗し、
新しい Decision、sidecar、互換 response を成功として残さない。既存旧recordの read は
sidecar 不在でも継続できる。
...
| AC-04 | 旧response fixture、validator拒否、SSOT publish failure後の全体rollback |
