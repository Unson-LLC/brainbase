...
- runtime 2.4以降も`PostToolUse`はcompleted stateをjournalへ記録するだけで、`Stop`を正規の確定境界とする。最終assistant回答の完全な監査ブロックを検証して`owner_audit_source=assistant_answer`と正確な`answer_digest`を記録する。
