...
Targeted test: `tests/conformance/brainbase-company-authority-producer-contract.test.js`. Initial TDD RED is the test-first invocation before the producer artifacts existed; GREEN is the same contract test after materializing the schema, reference validator, signed fixtures, and manifest. A successful targeted test proves contract conformance only, not production deployment or real authority resolution.
