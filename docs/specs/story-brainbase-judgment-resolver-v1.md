...
- terminal Stop: missing identity or missing episode exits non-zero without a final receipt; repairable required-capability or owner-visible-prefix omissions return one `decision:block`, then an incomplete active retry exits non-zero with `judgment_stop_repair_exhausted`
