...
Input includes the session/turn binding, `stop_hook_active`, and optional answer text. If the episode is missing, the Host may use only a complete `codex_app` `create_thread` or `send_message_to_thread` delegation envelope for the same turn and one connected session-alias component. Multiple candidates, foreign session components, malformed envelopes, or other tools fail closed as true orphans. A recovered route is explicitly post-generation and cannot be represented as a normal pre-generation episode. The Host evaluates required capabilities against immutable events, binds the exact business answer, and renders the stored owner judgment plus every stored tool-event line in atomic journal-commit order as its own `systemMessage`; the model does not reproduce that audit namespace. For a `continue` receipt, an unnecessary user question produces one short `🔁` in-progress `systemMessage` and an immutable structured continuation record. Episode start, event commits, and Stop finalization for the same turn share one per-turn SQLite `BEGIN IMMEDIATE` transaction. The Host prefers Node's built-in SQLite so Codex and shell processes with different CPU/ABI runtimes share the same portable implementation; Node 20 falls back to the locally installed `better-sqlite3` build. The OS releases the transaction lock on process exit, so the Host never reclaims or deletes a guessed-stale process lock file. Missing required knowledge or another substantive contract gap returns `decision:block` only on the first repairable Stop and writes no complete final receipt. If the active retry remains incomplete, the Host converges to `audit_degraded` instead of blocking again. Transaction-acquisition failure follows the terminal fail-closed boundary.
...
At Stop, the Host creates one immutable complete final receipt only after the contract is satisfied. When required knowledge or another substantive capability is absent, the first repairable Stop returns `decision:block` with a continuation reason. If the active repeated Stop is still incomplete, it finalizes as `audit_degraded` and exits 0 rather than blocking again. A true orphan Stop instead requests one degraded warning and answer-body preservation, then records non-final `audit_degraded` and exits 0 without `.final.json` or a new-task instruction; another first-phase payload cannot reopen the repair loop. Missing identity, immutable diagnostic conflict, diagnostic integrity failure, and transaction-acquisition timeout are explicit non-zero hook failures. When knowledge is optional and zero Brainbase calls were recorded, the Host-rendered audit block contains `📚 Brainbase未参照: 必須参照なし・実呼び出し0回 ✓`. A replay reuses an existing complete final. The final records `owner_audit_complete=true`, the expected Host-rendered line count, and an answer digest bound to the exact Stop Hook-visible business answer before the Host adds its audit namespace; transcript comparison may exclude only one complete trailing `<oai-mem-citation>...</oai-mem-citation>` block added later by the Codex application. Incomplete, embedded, or multiple citation blocks remain part of the answer body and fail closed on mismatch.
...
- S-003 `workflow state transition`: required capabilityを満たすopen episodeの`Stop`は、ordered event setとbusiness answer digestを束縛したcomplete finalへ1回だけ遷移し、owner-visible audit blockをHostの`systemMessage`として描画する。
- S-004 `workflow state transition`: required capabilityが不足する最初の`Stop`は`decision:block`でcontinuationを要求し、なお不完全な`stop_hook_active=true`の再Stopは`audit_degraded`へ収束する。
