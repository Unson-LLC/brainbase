...
- Connector-owned source identity must distinguish source-defined reruns that can change immutable run fields. GitHub Actions therefore uses `github:<run_id>:attempt:<run_attempt>` as `run.external_run_id`; only redelivery of the same attempt is a delivery retry. Each source connector Story owns an equivalent deterministic rule and a pre-fix rerun fixture.
...
- AC7/AC10 connector compatibility: `tests/connectors/github-actions-run-receipt.test.js` proves attempt-scoped GitHub identity, same-attempt delivery duplicate behavior, all reachable conclusion mappings, and unknown/null fallback without false success. The other three production connector Stories own equivalent source-identity, status, outbox, and evidence fixtures in their implementation repositories.
