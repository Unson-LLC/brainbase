...
- writer明示回復: `npm run canonical-task:recover-writer -- --expected-token <旧token> --new-token <新process token>`
...
3. 稼働予定processの`BRAINBASE_SERVER_GENERATION`を新tokenとして固定し、
   `npm run canonical-task:recover-writer -- --expected-token <旧token> --new-token <新token>`を実行する。
