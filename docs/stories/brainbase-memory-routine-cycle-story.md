...
- [x] `/ohayo`は全Googleアカウントと3 Slackワークスペースの確認範囲を入力として受け、未確認を0件にせず`partial`へ落とす。
- [x] `/ohayo`は想起順を今日の焦点にせず、今日の到達点、AIが進めること、人間が決めることを分ける。
- [x] `/ohayo`は要約外の予定・メール・Slackも全件を辿れる日別HTMLとして保存する。
...
- [x] おやすみレポートは`/oyasumi`の成果物として保存し、`/ohayo`の表示契約を変更しない。
