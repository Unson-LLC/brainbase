...
9. `brainbase onboard:install --target codex|claude|codecode --dry-run` emits a launchable client-specific MCP config without requiring users to guess a global binary path: Codex gets TOML, Claude and CodeCode get standard MCP JSON. `--output` writes a new snippet and refuses to overwrite existing client config.
