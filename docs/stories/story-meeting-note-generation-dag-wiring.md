...
2. **ingest後の生成dispatch**: Review Package ingest成功後、`transcript_to_meeting_note` loop intentをEve sessionへbest-effortで自動dispatchし、結果をrun/auditに記録する。
...
- INV-note-dag-003: Review Package ingestは生成dispatchの失敗・未設定で失敗してはいけない。dispatchはbest-effortとし、結果（requested / skipped + reason）をingestレスポンスとaudit logに記録する。
- INV-note-dag-004: Eve session client未設定時はdispatchをskipとして記録し、既存の手動dispatch API（`/control/loop-intents/:id/eve-session`）で後から再開できる状態を維持する。
...
flowchart TD
  ...
  ingest --> dispatch["auto dispatch transcript_to_meeting_note → Eve"]
  dispatch -->|configured| eve["Eve session generates minutes"]
  dispatch -->|unconfigured/failed| skipped["dispatch skipped (audit)"]
...
- S-002: Review Package ingest auto-dispatches the note generation loop intent to Eve when configured, and records a skip without failing when not configured.
...
- E2E contract: `tests/e2e/story-meeting-note-generation-dag-wiring-contract.spec.ts`
