...
### 0.3 環境構成（worktreeセッションのフレーム）

**重要**: Codex CLIはworktree環境で動作することを常に意識する。

#### 現在の位置

Codex CLIセッション開始時、以下の環境変数が設定される：

| 環境変数 | 説明 | 例 |
|---------|------|-----|
| `BRAINBASE_SESSION_ID` | セッションID | `session-1771656200964-brainbase` |
| `BRAINBASE_SERVER_PATH` | サーバーコードパス | `/Users/ksato/workspace/code/brainbase` |
| `PWD` | 現在のworktreeパス | `/Users/ksato/workspace/brainbase-config/.worktrees/session-*` |
...
- `default@` (サーバー): `$BRAINBASE_SERVER_PATH` で実行
- `session-*@` (このworktree): `$PWD` で開発

#### 編集場所と反映先

| 対象 | 編集場所 | 反映先 |
|------|---------|--------|
| サーバー動作 | `$BRAINBASE_SERVER_PATH` | launchd起動中 |
| worktree開発 | `$PWD` (このディレクトリ) | jj → PR → マージ後 |
| tmux設定 | `~/.tmux.conf` | 全セッション共通 |

**重要**: サーバー動作に関わる設定（tmux statusline等）は `$BRAINBASE_SERVER_PATH` で編集すること。
...
## 10. Codex Agent Addendum (Codex CLI最適化)

このセクションはCodex CLI実行環境での最適化ルールです。上記の開発標準に加えて、以下を優先します。

### 10.1 コマンド・探索
- 文字列検索/ファイル探索は `rg` / `rg --files` を優先
- `ls`/`cat`/`rg` など読み取り系コマンドは最小限で実行
- ネットワークアクセスは制限される前提で設計（必要なら承認を取得）

### 10.2 編集方針
- 変更は最小差分で実施
- 単一ファイルの編集は `apply_patch` を優先（自動生成や大規模置換は除外）
- 既存スタイルに合わせ、不要な整形や再フォーマットを避ける
- 予期しない変更を検知したら即停止し、ユーザーに確認する

### 10.3 安全性・Git運用
- 破壊的コマンド（`git reset --hard`、`git checkout --`、`rm -rf`等）は明示的に依頼された場合のみ
- 既存の未関連差分は勝手に戻さない
- コミット/マージはユーザー指示がある場合のみ実行
