 ...
 */
export function findFirstTable(content) {
 ...
 */
export function parseAppsTable(content) {
 ...
 */
export function parseCustomersTable(content) {
    const rows = parseMarkdownTable(content);
    return rows.map(row => ({
        name: row['顧客'] || '',
 ...
 */
export function parseRACITable(content) {
    const rows = parseMarkdownTable(content);
    return rows.map(row => ({
        item: row['項目'] || '',
 ...
 */
export function parseTablesBySection(content) {
 ...
 * | 人 | 資産 | 権利の範囲 |
 */
export function parsePositionTable(content) {
 ...
 * | 領域 | 決裁者 |
 */
export function parseDecisionTable(content) {
    const sections = parseTablesBySection(content);
 ...
 * | 人 | 領域 |
 */
export function parseAssignmentTable(content) {
    const sections = parseTablesBySection(content);
 ...
 */
export function parseProductsList(content) {
    const lines = content.split('\n');
    const products = [];
